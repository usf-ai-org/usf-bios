...
Trained Model Link
- [Simple-VL-8B](https://huggingface.co/usf/Simple-VL-8B/summary)
...
```python
import torch
from transformers import Qwen2_5_VLForConditionalGeneration, AutoModelForCausalLM, AutoConfig
...
### Inference
Perform inference using `usf infer`:
```bash
usf infer \
...
### Evaluation
Evaluate the trained VL model using [EvalScope](https://github.com/us-inc/evalscope/).
