...
from usf_bios.utils import is_debug_mode, HfConfigFactory, get_logger, init_process_group, set_default_ddp_config, to_abspath
...
class ExportArguments(MergeArguments, BaseArguments):
    ...
    def _init_output_dir(self):
        ...
        if is_debug_mode():
            logger.info_debug(f'args.output_dir: `{self.output_dir}`')
    ...
    def __post_init__(self):
        ...
        if self.to_mcore or self.to_hf:
            if self.merge_lora:
                ...
                logger.warning('`usf export --to_mcore/to_hf` does not support the `--merge_lora` parameter. '
