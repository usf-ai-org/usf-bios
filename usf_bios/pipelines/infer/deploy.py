...
from usf_bios.arguments import DeployArguments, InferArguments
from usf_bios.infer_engine import AdapterRequest, InferClient
from usf_bios.infer_engine.protocol import (ChatCompletionRequest, CompletionRequest, EmbeddingRequest, Model, ModelList,
                                         MultiModalRequestMixin)
from usf_bios.metrics import InferStats
from usf_bios.utils import is_debug_mode, JsonlWriter, get_logger
from .infer import USFInfer

logger = get_logger()


class USFDeploy(USFInfer):
    ...
    def get_infer_engine(args: InferArguments, template=None, **kwargs):
        if isinstance(args, DeployArguments) and args.infer_backend == 'vllm' and args.vllm_data_parallel_size > 1:
            ...
            logger.info_debug(f'Enable vLLM data parallel with size {args.vllm_data_parallel_size}.')
        return USFInfer.get_infer_engine(args, template, **kwargs)
    ...
    def _compute_infer_stats(self):
        ...
        logger.info_debug(global_stats)
    ...
    def _post_process(self, request_info, response, return_cmpl_response: bool = False):
        ...
        if is_finished:
            ...
            if self.args.verbose:
                logger.info_debug(request_info)
        return response
    ...
    def run(self):
        ...
        logger.info_debug(f'model_list: {self._get_model_list()}')
        uvicorn.run(
            self.app,
            host=args.host,
            port=args.port,
