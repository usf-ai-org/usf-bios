...
def do_sample(model: str, model_type: str, dataset: List[str], iter: int):
    ...
    for device in range(device_count):
        sample_cmd = (f'{conda_prefix} USE_OPENCOMPASS_EVALUATOR=True CUDA_VISIBLE_DEVICES={device} usf sample '
    ...
    for device in range(device_count):
        sample_cmd = (
            f'{conda_prefix} USE_OPENCOMPASS_EVALUATOR=True CUDA_VISIBLE_DEVICES={device} usf sample '
...
def do_train(model: str, model_type: str, datasets: List[str], iter, cmd='sft'):
    ...
    extra_args = ''
    if cmd == 'rlhf':
        extra_args = '--rlhf_type dpo --beta 0.3 '  # use another reinforce learning method supported by usf
    ga = 128 // get_device_count() // 2
    train_cmd = (f'{conda_prefix} {gpu_prefix} usf {cmd} '
                 f'--model {model} --model_type {model_type} '
...
def replace_math_dataset():
    ...
    # and one must run usf eval --eval_dataset math first to make sure opencompass has created
    # the folder.
