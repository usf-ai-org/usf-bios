...
from usf_bios.tuners.utils import ModulesToSaveWrapper
...
class TestExtraStateDict(unittest.TestCase):
    ...
    def test_usf_bios_extra_state_dict(self):
        model = AutoModel.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        lora_config = LoRAConfig(target_modules=['query', 'key', 'value'])
        model = USF_BIOS.prepare_model(model, lora_config, extra_state_keys=['classifier.*'])
        ...
        model = AutoModel.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        model = USF_BIOS.from_pretrained(model, self.tmp_dir, inference_mode=False)
    ...
    def test_usf_bios_modules_to_save(self):
        model = AutoModel.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        ...
        model = USF_BIOS.prepare_model(model, {'lora1': lora_config, 'lora2': lora_config2})
        ...
        model = AutoModel.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        model = USF_BIOS.from_pretrained(model, self.tmp_dir, adapter_name='lora2')
        names = [name for name, value in model.named_parameters() if value.requires_grad]
        self.assertTrue(any('classifier' in name for name in names))
        self.assertTrue(
