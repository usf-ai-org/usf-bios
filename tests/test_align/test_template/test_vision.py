...
def _infer_model(engine, system=None, messages=None, images=None, **kwargs):
    ...
    if images is None:
        images = ['https://huggingface.co/datasets/usf-bios/test-assets/resolve/main/cat.png']
...
def test_qwen2_5_vl_batch_infer():
    ...
    resp = engine.infer([{
        ...
        'videos': ['https://huggingface.co/datasets/usf-bios/test-assets/resolve/main/baby.mp4']
    }],
    ...
    messages2 = [{
        ...
        'content': [
            {
                'type': 'video',
                'video': 'https://huggingface.co/datasets/usf-bios/test-assets/resolve/main/baby.mp4'
            },
            {
                'type': 'text',
...
def test_qwen3_omni_audio():
    ...
    audios = ['https://huggingface.co/datasets/usf-bios/test-assets/resolve/main/weather.wav']
...
(Answer in Action-Operation-Sensitive format.)"""
    ...
    images = ['https://huggingface.co/datasets/usf-bios/test-assets/resolve/main/agent.png']
    response = _infer_model(engine, messages=messages, images=images)
    engine.template.template_backend = 'jinja'
...
def test_minicpmo():
    ...
    images = [
        'https://huggingface.co/datasets/usf-bios/test-assets/resolve/main/cat.png',
        'https://huggingface.co/datasets/usf-bios/test-assets/resolve/main/animal.png'
    ]
    response = _infer_model(engine, messages=messages, images=images)
    engine.template.template_backend = 'jinja'
    response2 = _infer_model(engine, messages=messages, images=images)
...
def test_got_ocr():
    # https://github.com/us-inc/usf-bios/issues/2122
    ...
    _infer_model(
        ...
        images=['https://huggingface.co/datasets/usf-bios/test-assets/resolve/main/ocr.png'])
...
def test_got_ocr_hf():
    ...
    response = _infer_model(
        ...
        images=['https://huggingface.co/datasets/usf-bios/test-assets/resolve/main/ocr.png'])
    assert response[:200] == ('简介 USF BIOS支持250+LLM和35+MLLM（多模态大模型）的训练、推理、 评测和部署。开发者可以直接将'
...
def test_florence():
    engine = TransformersEngine('microsoft/Florence-2-base-ft')
    ...
    response = _infer_model(
        ...
        images=['https://huggingface.co/datasets/usf-bios/test-assets/resolve/main/animal.png'])
...
def test_paligemma():
    engine = TransformersEngine('google/paligemma-3b-mix-224')
    response = _infer_model(engine, messages=[{'role': 'user', 'content': 'detect cat'}])
    assert response == '<loc0000><loc0000><loc1022><loc1022> cat'


def test_paligemma2():
    engine = TransformersEngine('google/paligemma2-3b-ft-docci-448', torch_dtype=torch.bfloat16)
...
def test_pixtral():
    engine = TransformersEngine('mistralai/Pixtral-12B-2409')
    _infer_model(engine, messages=[{'role': 'user', 'content': '<image>这是什么'}])
...
def test_doc_owl2():
    ...
    images = [
        'https://huggingface.co/mPLUG/DocOwl2/resolve/main/examples/docowl2_page0.png',
        'https://huggingface.co/mPLUG/DocOwl2/resolve/main/examples/docowl2_page1.png',
        'https://huggingface.co/mPLUG/DocOwl2/resolve/main/examples/docowl2_page2.png',
        'https://huggingface.co/mPLUG/DocOwl2/resolve/main/examples/docowl2_page3.png',
        'https://huggingface.co/mPLUG/DocOwl2/resolve/main/examples/docowl2_page4.png',
        'https://huggingface.co/mPLUG/DocOwl2/resolve/main/examples/docowl2_page5.png',
    ]
    response = _infer_model(
        engine,
        messages=[{
...
def test_keye_vl():
    ...
    engine.template.template_backend = 'usf_bios'
    response = _infer_model(engine, messages=messages, images=images)
    engine.template.template_backend = 'jinja'
    response2 = _infer_model(engine, messages=messages, images=images)
    assert response == response2
...
def test_deepseek_ocr():
    ...
    assert response == ('# 简介\n\nUSF BIOS支持250+ LLM和35+ MLLM（多模态大模型）的训练、推理、评测和部署。开发者可以直接'
                        ...
                        '\nUSF BIOS具有丰富的文档体系，如有使用问题请查看这里。\n\n可以在HuggingFace Spaces'
                        '创空间 中体验USF BIOS web-ui功能了。')
...
def test_paddle_ocr():
    ...
    assert response == ('USF BIOS支持250+ LLM和35+ MLLM（多模态大模型）的训练、推理、评测和部署。开发者可以直接将我们的框架'
                        ...
                        '系，如有使用问题请请查看这里。\n\n可以在Huggingface space 和 HuggingFace Spaces 中体验USF BIOS web'
                        '-ui功能了。')
...
if __name__ == '__main__':
    from usf_bios.infer_engine import TransformersEngine, RequestConfig
    from usf_bios.utils import get_logger, seed_everything
