...
def test_llm():
    from usf_bios import rlhf_main, RLHFArguments, infer_main, InferArguments
    result = rlhf_main(
        RLHFArguments(
            rlhf_type='grpo',
...
def test_llm_vllm():
    ...
    result = rlhf_main(
        RLHFArguments(
            rlhf_type='grpo',
            model='Qwen/Qwen2.5-1.5B-Instruct',
            reward_model='Ray2333/GRM-Llama3.1-8B-rewardmodel-ft',
            train_type='full',
            dataset=['AI-MO/NuminaMath-TIR#100'],
            system=SYSTEM_PROMPT,
            reward_funcs=['accuracy', 'format'],
...
def test_mllm_pt():
    ...
    result = rlhf_main(
        RLHFArguments(
            ...
            train_type='full',
            # dataset=['AI-MO/NuminaMath-TIR#100'],
            dataset=['detection-datasets/coco:validation#100'],
            system=SYSTEM_PROMPT,
            reward_funcs=['format'],
            max_completion_length=4096,
            num_generations=2,
