...
from usf_bios.model import MODEL_MAPPING
from usf_bios.rlhf_trainers import GRPOArgumentsMixin
from usf_bios.trainers import RLHFArgumentsMixin
from usf_bios.utils import is_debug_mode, get_current_device, get_logger, is_master, is_mp, json_parse_to_dict, set_default_ddp_config
...
class GRPOArguments(GRPOArgumentsMixin):
    ...
    Args:
        ...
        reward_funcs (List[str]): A list of reward function names to use for the GRPO algorithm. Available built-in
            ...
            (see usf_bios/plugins/orm.py). Custom reward functions can also be defined. Defaults to an empty list.
...
class RLHFArguments(TeacherModelArguments, GRPOArguments, PPOArguments, RewardModelArguments, RLHFArgumentsMixin,
    ...
    Args:
        ...
        rpo_alpha (Optional[float]): The alpha parameter from the RPO paper, controlling the weight of the SFT loss
            ...
            included. Note: The default was 1.0 in `usf-bios<3.8` and changed to None in `usf-bios>=3.8`. Defaults to
            None.
    ...
    def _init_grpo(self):
        ...
        logger.info_debug(f'Setting args.remove_unused_columns: {self.remove_unused_columns}')
        ...
        if self.async_generate:
            logger.info_debug('Using async mode. This is a approximate version which '
        ...
        if 'soft_overlong' in self.reward_funcs:
            ...
            if self.soft_max_length is None:
                ...
                logger.info_debug(f'Auto-configured soft_max_length = max_completion_length {self.max_completion_length}')
    ...
    def _init_rollout(self):
        ...
        if self.async_generate:
            ...
            'please deploy vLLM server by `usf rollout` and assign with `vllm_server_host` '
            'for more infomations, please check '
            'https://usf-bios.readthedocs.io/en/latest/Instruction/GRPO/getstarted/GRPO.html'
    ...
    def _init_external_vllm(self):
        if self.rlhf_type not in rlhf_support_vllm_types or (self.vllm_server_host is None
            ...
            return
        from usf_bios.rlhf_trainers import VLLMClient
        if is_master():
            logger.info_debug('Start connecting to vLLM server')
            ...
            logger.info_debug('Connected to vLLM server')
    ...
    def _set_default(self):
        ...
        if self.gradient_accumulation_steps is None:
            if self.rlhf_type == 'grpo':
                ...
                logger.info_debug('Setting default gradient_accumulation_steps to 1 for GRPO.')
        ...
        if self.use_liger_kernel:
            ...
            if self.importance_sampling_level != 'token':
                ...
                if self.importance_sampling_level == 'sequence_token':
                    ...
                    logger.info_debug('Remapping `importance_sampling_level` from `sequence_token` to `sequence` for '
    ...
    def _external_vllm_warning(self):
        ...
        if self.vllm_max_model_len is not None:
            logger.warning(
                ...
                '`usf rollout --vllm_max_model_len <value>`', self.vllm_max_model_len)
    ...
    def _init_teacher_deepspeed(self):
        ...
        if is_debug_mode():
            logger.info_debug(f'Using teacher_deepspeed config: {self.teacher_deepspeed}')
