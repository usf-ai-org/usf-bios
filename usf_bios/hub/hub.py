...
from usf_bios.utils import get_logger, ms_logger_context, use_hf_hub
...
class MSHub(HubOperation):
    ...
    def create_model_repo(cls, repo_id: str, token: Optional[str] = None, private: bool = False) -> str:
        ...
        if not cls.try_login(token):
            raise ValueError('Please specify a token by `--hub_token` or `HF_TOKEN=xxx`')
        cls.ms_token = token
        ...
        if '/' not in repo_id:
            ...
            logger.info_debug(f"'/' not in hub_model_id, pushing to personal repo {hub_model_id}")
        try:
    ...
    def download_model(cls,
        ...
        if revision is None or revision == 'main':
            revision = 'master'
        logger.info_debug(f'Downloading the model from HuggingFace Hub, model_id: {model_id_or_path}')
