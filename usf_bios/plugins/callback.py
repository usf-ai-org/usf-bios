...
from usf_bios.utils import is_debug_mode, empty_cache, get_logger
...
class EarlyStopCallback(TrainerCallback):
    ...
    def on_save(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        if self.interval >= self.total_interval:
            logger.info_debug(f'Training stop because of eval metric is stable at step {state.global_step}')
...
class PerfMetricsLogCallback(TrainerCallback):
    ...
    def on_init_end(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        from usf_bios.utils import get_current_device, get_device_count, get_env_args

        # Top priority. Specify by ENV
        tflops = get_env_args('DEVICE_TFLOPS', int, None)
        device_count = max(get_device_count(), 1)
        if tflops is not None:
            logger.info_debug(f"Specify theoretical max TFLOPS through ENV 'DEVICE_TFLOPS'. [{tflops} TFLOPS]")
        else:
            ...
            if is_debug_mode():
                if is_debug_mode():
                    logger.info_debug(f'Estimating device TFLOPS baseline. Device: [{device}] dtype: [{dtype}]')
            tflops = self._estimate_device_tflops_by_dtype(device, dtype)
            if is_debug_mode():
                if is_debug_mode():
                    logger.info_debug(f'Estimate test finished. [{tflops} TFLOPS] Device count: [{device_count}]')
    ...
    def _estimate_device_tflops_by_dtype(device: torch.device, dtype: torch.dtype, repeats: int = 60, dim: int = 8192):
        ...
        if is_debug_mode():
            if is_debug_mode():
                logger.info_debug(f'[Device {device}] Total time: {total_time:.4f}s, dtype: {dtype}, Perf: {tflops:.4f} TFLOPS')
        return tflops
