...
from usf_bios.trainers.arguments import TrainArgumentsMixin
from usf_bios.utils import (add_version_to_work_dir, get_device_count, get_logger, get_pai_tensorboard_dir,
                         is_debug_mode, is_master, is_mp, is_pai_training_job, is_swanlab_available,
                         json_parse_to_dict, to_abspath)
...
class SftArguments(SwanlabArguments, TunerArguments, BaseArguments, TrainArgumentsMixin, Seq2SeqTrainingArguments):
    ...
    def _init_deepspeed(self):
        if self.deepspeed:
            ...
            logger.info_debug(f'Using deepspeed: {self.deepspeed}')
    ...
    def _init_fsdp(self):
        ...
        logger.info_debug(f'Using FSDP2: fsdp={self.fsdp}, fsdp_config={self.fsdp_config}')
    ...
    def _handle_pai_compat(self) -> None:
        ...
        logger.info_debug('Handle pai compat...')
        ...
        if self.logging_dir is None and pai_tensorboard_dir is not None:
            ...
            logger.info_debug(f'Setting args.logging_dir: {self.logging_dir}')
        self.add_version = False
        logger.info_debug(f'Setting args.add_version: {self.add_version}')
    ...
    def _add_version(self):
        ...
        if self.add_version:
            ...
            if is_debug_mode():
                logger.info_debug(f'output_dir: {self.output_dir}')
    ...
    def _init_eval_strategy(self):
        ...
        if self.eval_strategy == 'no':
            ...
            if self.split_dataset_ratio > 0:
                ...
                logger.info_debug(f'Setting args.split_dataset_ratio: {self.split_dataset_ratio}')
