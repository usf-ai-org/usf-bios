...
Currently, USF BIOS supports interface-based training and inference, with parameter support similar to script training. After installing USF BIOS, use the following command:
...
USF BIOS web-ui is a high-level wrapper for the command line. In other words, tasks such as training and deployment initiated through the interface will start an independent process in the system via the command line. Pseudo-code is similar to:

```python
import os
os.system('usf sft --model xxx --dataset xxx')
```
...
6. Training through the web-ui does not support PPO, as the process is more complex. It is recommended to use the [shell script](https://github.com/us-inc/usf-bios/tree/main/examples/train/rlhf/ppo) in the examples directory to run it directly.
...
Additionally, usf-bios supports interface inference mode (i.e., Space deployment):

```shell
usf app --model '<model>' --studio_title My-Awesome-Space --stream true
# or
usf app --model '<model>' --adapters '<adapter>' --studio_title My-Awesome-Space --stream true
```
