"""
EvalScope integration utilities for usf-bios models.

This module provides a custom ModelAPI implementation that enables batch inference
for evaluation tasks using usf-bios's TransformersEngine. It implements an asynchronous
...
class BatchInferInput:
    ...
    """
    ms_input: InferRequest  # usf-bios format request
    ms_config: RequestConfig  # usf-bios format configuration
...
@register_model_api('usf_custom')
class EvalModel(ModelAPI):
    """
    Custom ModelAPI implementation for usf-bios models with batch inference support.

    This class integrates usf-bios's TransformersEngine with EvalScope's evaluation framework,
    ...
    def __init__(
        ...
        """
        Initialize the EvalModel with usf-bios backend.
    ...
    def generate(
        ...
        # Convert EvalScope format to usf-bios format
...
def _process_batches() -> None:
    ...
    while True:
        ...
        try:
            # Prepare batch inputs for usf-bios inference
            ...
            # Perform batch inference using usf-bios engine
            ...
            for i, (batch_input, future) in enumerate(inputs):
                ...
                # Convert usf-bios response to EvalScope format
...
def convert_config(config: GenerateConfig) -> RequestConfig:
    """
    Convert EvalScope GenerateConfig to usf-bios RequestConfig.
    ...
    Returns:
        RequestConfig: usf-bios compatible configuration
    """
...
def convert_request(messages: List[EvalChatMessage], tools: List[ToolInfo]) -> InferRequest:
    """
    Convert EvalScope request format to usf-bios InferRequest format.

    Transforms the message and tool format from EvalScope's representation
    to the format expected by usf-bios's inference engine.
    ...
    Returns:
        InferRequest: usf-bios compatible request object
    """
    # Convert tools to usf-bios format
    tools_list = []
    if len(tools) > 0:
        tools_list = [tool.model_dump(exclude_none=True) for tool in tools]

    # Convert messages to usf-bios format
    ms_messages = []
