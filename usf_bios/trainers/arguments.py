...
from usf_bios.optimizers.galore import GaLoreConfig
from usf_bios.utils import get_dist_setting, get_logger, is_liger_available, is_mp, json_parse_to_dict
...
class TrainArgumentsMixin:
    ...
    def _patch_liger_kernel():
        ...
        logger.info_debug('Patch liger_kernel successfully.')
    ...
    def __post_init__(self):
        ...
        if self.gradient_accumulation_steps is None:
            ...
            logger.info_debug(f'Setting args.gradient_accumulation_steps: {self.gradient_accumulation_steps}')
        if self.lr_scheduler_kwargs:
            self.lr_scheduler_kwargs = json_parse_to_dict(self.lr_scheduler_kwargs)
        if 'wandb' in self.report_to:
            os.environ.setdefault('WANDB_PROJECT', 'usf-bios')
        ...
        if self.dataloader_num_workers is None:
            ...
            logger.info_debug(f'Setting args.dataloader_num_workers: {self.dataloader_num_workers}')
...
@dataclass
class USFArgumentsMixin(RLHFArgumentsMixin, TrainArgumentsMixin):
...
class VllmArguments:
    ...
    Args:
        ...
        vllm_use_async_engine (Optional[bool]): Whether to use async engine for vLLM. Default is None,
            ...
            deployment scenarios (usf deploy) and False otherwise.
...
@dataclass
class TrainingArguments(USFArgumentsMixin, HfTrainingArguments):
    pass


@dataclass
class Seq2SeqTrainingArguments(USFArgumentsMixin, HfSeq2SeqTrainingArguments):
    pass
