...
megatron sft \
    ...
    --dataset 'linxy/LaTeX_OCR:human_handwrite#5000' \
...
megatron sft \
    ...
    --load_safetensors true \
    --save_safetensors true \
    --dataset 'usf/Chinese-Qwen3-235B-Thinking-2507-Distill-data-110k-SFT#20000' \
    --load_from_cache_file true \
    --split_dataset_ratio 0.01 \
...
megatron sft \
    ...
    --save_safetensors true \
    --merge_lora false \
    --dataset 'usf/Chinese-Qwen3-235B-2507-Distill-data-110k-SFT#2000' \
              'usf_bios/self-cognition#1000' \
    --load_from_cache_file true \
    --train_type lora \
    --lora_rank 8 \
    --lora_alpha 32 \
...
In addition to supporting safetensors conversion and saving during training, Mcore-Bridge also supports the `megatron export` command for standalone weight export. `megatron export` supports conversion precision testing during weight conversion, which is very helpful for verifying accuracy when integrating new models. Typically, models already integrated into Megatron-USF BIOS will not have precision misalignment issues, so you can confidently set `--test_convert_precision false`.
...
from usf_bios.megatron import MegatronArguments, convert_hf_config, get_megatron_model_meta
from usf_bios.model import get_processor
