...
from usf_bios.utils import (append_to_jsonl, format_time, get_device_count, get_logger, get_torch_device, is_mp,
                         is_pai_training_job)
...
class DefaultFlowCallbackNew(DefaultFlowCallback):
    ...
    def on_epoch_end(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        if args.max_epochs is not None and args.max_epochs <= math.ceil(state.epoch):
            logger.info_debug('Training has reached `max_epochs`. The model will be saved and the training will be exited.')
            if evaluation_strategy != IntervalStrategy.NO:
                control.should_evaluate = True
            if args.save_strategy != IntervalStrategy.NO:
                control.should_save = True
