...
from usf_bios.infer_engine.protocol import RequestConfig, RolloutInferRequest, RolloutOutput
from usf_bios.megatron.arguments import MegatronArguments, MegatronRLHFArguments
from usf_bios.megatron.utils import MegatronTrainerState, forward_step_helper, get_padding_to
from usf_bios.plugins import MultiTurnScheduler, multi_turns, orms
from usf_bios.rlhf_trainers.grpo_trainer import DataType
from usf_bios.rlhf_trainers.utils import (aggressive_empty_cache, nanstd, pad_logps_back_to_batch,
                                       replace_assistant_response_with_ids, set_expandable_segments)
from usf_bios.template import Template, TemplateInputs
from usf_bios.utils import (get_logger, get_packed_seq_params, is_wandb_available, remove_response,
                         shutdown_event_loop_in_daemon, start_event_loop_in_daemon, to_device)
from .rlhf_mixin import MegatronRLHFTrainer
from .rollout_mixin import MegatronRolloutMixin
from .utils import gather, gather_object, get_usf_datasets_provider, profiling_context, profiling_decorator
...
class MegatronGRPOTrainer(MegatronRolloutMixin, MegatronRLHFTrainer):
    ...
    def train(self, train_dataset, val_dataset, data_collator):
        ...
        if self.dynamic_sample or self.truncation_strategy == 'raise':
            self._train_valid_test_dataset_provider = get_usf_datasets_provider(train_dataset, val_dataset)
        ...
        for _ in range(max_resample_rounds + 1):
            ...
            while pending_samples and len(valid_samples) < required_count:
                ...
                try:
                    ...
                    logger.info_debug(f'Encoding failed for one sample; will resample. {e}')
    ...
    def _prepare_metrics(self):
        args = self.args
        from usf_bios.utils import JsonlWriter
