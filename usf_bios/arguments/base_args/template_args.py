...
class TemplateArguments:
    ...
    Args:
        ...
        truncation_strategy (Literal['delete', 'left', 'right', 'split']): Strategy for handling samples exceeding
            ...
            Note: The 'split' strategy is only supported during pre-training (e.g., `usf/megatron pt`), requires
            `usf-bios>=3.11`, and is incompatible with `cached_dataset`. It splits long samples to avoid wasting
            tokens.
        ...
        use_chat_template (bool): Whether to use the chat template or the generation template. The generation template
            ...
            Note: Defaults to False for `usf pt`, which uses the generation template. This parameter is compatible
        ...
        loss_scale (str): Loss weight configuration for training tokens. Default is `'default'`.
            ...
            [loss_scale module](https://github.com/us-inc/usf-bios/blob/main/usf_bios/loss_scale/mapping.py).
            usf-bios>=3.12 supports mixing basic strategies with other strategies,
            ...
            - 'last_round': Only calculate loss for the last round response. In "usf-bios>=3.12", the last round
        ...
        sequence_parallel_size (int): The size of sequence parallelism. Defaults to 1. Currently supported for CPT,
            SFT, DPO, and GRPO.
        template_backend (Literal['usf_bios', 'jinja']): The backend to use for templating. Options are 'usf_bios' or
            'jinja'. Defaults to 'usf_bios'. If 'jinja' is used, it will leverage `transformers.apply_chat_template`.
        ...
        enable_thinking (Optional[bool]): (usf-bios>=3.12) This parameter takes effect during inference,
        ...
        add_non_thinking_prefix (bool): This parameter only takes effect during training, indicating whether to
            ...
            This feature allows usf's built-in datasets to train hybrid thinking models. Default value is True.
    ...
    def __post_init__(self):
        ...
        if self.padding_side is None:
            if getattr(self, 'task_type', None) in ('reranker', 'generative_reranker'):
                ...
                logger.info_debug(f'Setting args.padding_side to {self.padding_side} for task_type={self.task_type}')
            else:
