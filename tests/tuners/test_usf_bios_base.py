...
from transformers import SbertConfig, SbertForSequenceClassification
from peft import PeftModel
from peft.utils import SAFETENSORS_WEIGHTS_NAME
from torch import nn

from usf_bios.tuners import AdapterConfig, LoRAConfig, PromptConfig, ResTuningConfig, SideConfig, USF_BIOS, USFModel
from usf_bios.tuners.part import PartConfig
...
class TestUSF(unittest.TestCase):
    ...
    def test_usf_bios_lora_forward(self):
        ...
        from usf_bios.tuners.lora import Linear
    ...
    def test_usf_bios_adapter_forward(self):
        model = AutoModel.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        preprocessor = Preprocessor.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        inputs = preprocessor('how are you')
        adapter_config = AdapterConfig(
            ...
            hidden_pos=0)
        outputs = model(**inputs)
        model = USF_BIOS.prepare_model(model, config=adapter_config)
        outputs_lora = model(**inputs)
        model.deactivate_adapter('default')
        outputs_deactivate = model(**inputs)
        model.activate_adapter('default')
    ...
    def test_usf_bios_prompt_forward(self):
        model = AutoModel.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        preprocessor = Preprocessor.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        inputs = preprocessor('how are you')
        ...
        outputs = model(**inputs)
        model = USF_BIOS.prepare_model(model, config=prompt_config)
        outputs_lora = model(**inputs)
        model.deactivate_adapter('default')
        outputs_deactivate = model(**inputs)
        model.activate_adapter('default')
    ...
    def test_usf_bios_restuner_forward(self):
        model = AutoModel.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        preprocessor = Preprocessor.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        inputs = preprocessor('how are you')
        ...
        outputs = model(**inputs)
        model = USF_BIOS.prepare_model(model, config=restuner_config)
        outputs_lora = model(**inputs)
        model.deactivate_adapter('default')
        outputs_deactivate = model(**inputs)
        model.activate_adapter('default')
    ...
    def lora_injection_with_dtype(self, dtype=torch.float32):
        ...
        model2 = AutoModel.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        ...
        model2 = USF_BIOS.from_pretrained(model2, self.tmp_dir, adapter_name={'default': 'test'})
        self.assertTrue('test' in model2.adapters)
        output2 = model2(**input)
        self.assertTrue(torch.allclose(output1.logits, output2.logits))
        model2 = USF_BIOS.from_pretrained(model2, self.tmp_dir)
        ...
        if dtype == torch.float32 and os.environ.get('USE_UNIQUE_THREAD') == '1':
            USF_BIOS.merge_and_unload(model2)
            output3 = model2(**input)
            self.assertTrue(torch.allclose(output1.logits, output3.logits))

    def test_usf_bios_lora_injection(self):
        self.lora_injection_with_dtype()

    def test_usf_bios_lora_injection_bf16(self):
    ...
    def test_save_to_peft_mix(self):
        ...
        adapter_config = AdapterConfig(
            ...
            method_name='feed_forward_chunk',
            hidden_pos=0)
        model = USF_BIOS.prepare_model(model, config={'lora': lora_config, 'adapter': adapter_config})
        model.save_pretrained(os.path.join(self.tmp_dir, 'original'))
        try:
            USF_BIOS.save_to_peft_format(os.path.join(self.tmp_dir, 'original'), os.path.join(self.tmp_dir, 'converted'))
    ...
    def test_save_to_peft_param(self):
        ...
        model = USF_BIOS.prepare_model(model, config={'lora': lora_config})
        model.save_pretrained(os.path.join(self.tmp_dir, 'original'))
        try:
            USF_BIOS.save_to_peft_format(os.path.join(self.tmp_dir, 'original'), os.path.join(self.tmp_dir, 'converted'))
            self.assertTrue(False)
    ...
    def test_save_to_peft_ok(self):
        ...
        model = USF_BIOS.prepare_model(model, config={'default': lora_config, 'lora': lora2_config})
        model.save_pretrained(os.path.join(self.tmp_dir, 'original'))
        USF_BIOS.save_to_peft_format(os.path.join(self.tmp_dir, 'original'), os.path.join(self.tmp_dir, 'converted'))
        ...
        model2 = PeftAutoModel.from_pretrained(model2, os.path.join(self.tmp_dir, 'converted'))
        ...
        # -------------------override case--------------------
        USF_BIOS.save_to_peft_format(os.path.join(self.tmp_dir, 'converted'), os.path.join(self.tmp_dir, 'converted'))
        model2 = SbertForSequenceClassification(SbertConfig())
        model2 = PeftAutoModel.from_pretrained(model2, os.path.join(self.tmp_dir, 'converted'))
        model2.load_adapter(os.path.join(os.path.join(self.tmp_dir, 'converted'), 'lora'), 'lora')
        state_dict = model.state_dict()
    ...
    def test_usf_bios_multiple_adapters(self):
        ...
        model2 = USF_BIOS.from_pretrained(model2, self.tmp_dir, adapter_name=['lora', 'adapter'])
        state_dict = model.state_dict()
        state_dict2 = model2.state_dict()
        for key in state_dict:
            self.assertTrue(key in state_dict2)
    ...
    def test_part(self):
        ...
        model = USF_BIOS.prepare_model(model, config={'part': part_config})
        self.assertTrue(isinstance(model, USFModel))
        ...
        model2 = USF_BIOS.from_pretrained(model2, self.tmp_dir, adapter_name=['part'])
        self.assertTrue(
            all(
        ...
        model2 = USF_BIOS.prepare_model(model2, config={'part2': part_config})
        model2 = USF_BIOS.prepare_model(model2, config={'lora': lora_config})
        model2 = USF_BIOS.prepare_model(model2, config={'part3': part_config})
    ...
    def test_usf_bios_multiple_adapters_switching(self):
        from usf_bios.tuners.lora import Linear
        from usf_bios.tuners.adapter import AdapterModule
    ...
    def test_usf_bios_side_bert(self):
        model = AutoModel.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        preprocessor = Preprocessor.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        inputs = preprocessor('how are you')
        model2 = copy.deepcopy(model)
        result_origin = model(**inputs).logits
        print(f'test_usf_bios_side_bert result_origin shape: {result_origin.shape}, '
        ...
        model = USF_BIOS.prepare_model(model, config=side_config)
        ...
        print(f'test_usf_bios_side_bert result shape: {result_origin.shape}, result sum: {torch.sum(result_origin)}')
