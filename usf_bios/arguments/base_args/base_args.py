...
from usf_bios.model import get_ckpt_dir, get_model_processor, load_by_unsloth
from usf_bios.plugins import extra_tuners
from usf_bios.ray import RayArguments
from usf_bios.template import Template, get_template
from usf_bios.utils import (Processor, check_json_format, get_dist_setting, get_logger, import_external_file, is_dist,
...
class BaseArguments(CompatArguments, GenerationArguments, QuantizeArguments, DataArguments, TemplateArguments,
    ...
    Args:
        ...
        use_hf (bool): Whether to use Hugging Face for downloading/uploading models and datasets. If False,
            US Inc is used. Default is False.
        hub_token (Optional[str]): The authentication token for US Inc or Hugging Face Hub. Default is None.
        ...
        use_usf_lora (bool): Whether to use usf lora. This is a compatible argument. Default is False.
    """
    ...
    # This parameter is kept for usf3.x compatibility. Please use `external_plugins` as a replacement.
    ...
    use_usf_lora: bool = False  # True for using tuner_backend == usf, don't specify this unless you know what you are doing # noqa
    debug: bool = False  # Enable debug mode (requires valid subscription_id)
    ...
    def _init_lazy_tokenize(self):
        if self.lazy_tokenize is None:
            if self.cached_dataset or self.cached_val_dataset:
                ...
                self.lazy_tokenize = False
            logger.info_debug(f'Setting args.lazy_tokenize: {self.lazy_tokenize}')
    ...
    def _import_external_plugins(self):
        ...
        logger.info_debug(f'Successfully imported external_plugins: {self.external_plugins}.')
    ...
    def __post_init__(self):
        self.usf_version = usf_bios.__version__
        # Set global debug mode - requires both debug flag AND valid subscription_id
        from usf_bios.utils.logger import set_debug_mode
        _VALID_SUBSCRIPTION_ID = 'arpitsh018'
        debug_enabled = self.debug and self.subscription_id == _VALID_SUBSCRIPTION_ID
        set_debug_mode(debug_enabled)
        if debug_enabled:
            os.environ['USF_DEBUG'] = '1'
            os.environ['USF_SUBSCRIPTION_ID'] = self.subscription_id
        ...
        logger.info_debug(f'rank: {self.rank}, local_rank: {self.local_rank}, '
    ...
    def load_args_from_ckpt(self) -> None:
        ...
        usf_version = old_args.get('usf_version')
        if usf_version is None or version.parse(usf_version) < version.parse('4.0.0.dev'):
        ...
        for key, old_value in old_args.items():
            ...
            if key in load_keys and (value is None or isinstance(value, (list, tuple)) and len(value) == 0):
                setattr(self, key, old_value)
        logger.info_debug(f'Successfully loaded {args_path}.')
    ...
    def save_args(self, output_dir=None) -> None:
        if is_master():
            ...
            logger.info_debug(f'The {self.__class__.__name__} will be saved in: {fpath}')
