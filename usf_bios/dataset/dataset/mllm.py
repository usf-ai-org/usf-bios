...
from usf_bios.utils import get_hf_endpoint, use_hf_hub
...
class ShareGPT4oPreprocessor(MessagesPreprocessor):
    ...
    def prepare_dataset(self, dataset):
        if not use_hf_hub():
            url = ('https://www.modelscope.cn/api/v1/datasets/ShareGPT-4o/repo?'
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/gpt4v-dataset',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/RLAIF-V-Dataset',
...
class SA1BPairedCaptionPreprocessor(RowPreprocessor):
    ...
    def preprocess(self, row: Dict[str, Any]) -> Dict[str, Any]:
        prompt = ['What is shown in the image', 'Describe the content of the image', 'Tell me what is inside', 'What is the image content']
...
class MantisPreprocessor(MessagesPreprocessor):
    ...
    def prepare_dataset(self, dataset: HfDataset) -> HfDataset:
        if not use_hf_hub():
            url = (f'https://www.us.inc/api/v1/datasets/usf_bios/Mantis-Instruct/repo?Revision='
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/Mantis-Instruct',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/llava-data',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/pixelprose',
...
class EmoSchemaPreprocessor(ResponsePreprocessor):
    ...
    def prepare_dataset(self, dataset: HfDataset) -> HfDataset:
        for i in range(1, 6):
            if not use_hf_hub():
                url = f'https://modelscope.cn/datasets/egoschema/resolve/master/videos_chunked_0{i}.zip'
            else:
...
class MovieChat1KPreprocessor(ResponsePreprocessor):
    ...
    def prepare_dataset(self, dataset: HfDataset) -> HfDataset:
        ...
        for file in mp4_set:
            if not use_hf_hub():
                url = f'https://modelscope.cn/datasets/Enxin/MovieChat-1K-test/resolve/master/videos/{file}'
            else:
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='Enxin/MovieChat-1K-test',
...
class VideoChatGPTPreprocessor(ResponsePreprocessor):
    ...
    def prepare_dataset(self, dataset: HfDataset) -> HfDataset:
        if not use_hf_hub():
            url = 'https://modelscope.cn/datasets/usf_bios/VideoChatGPT/resolve/master/videos.zip'
        else:
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/VideoChatGPT',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/Multimodal-Mind2Web',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='Lin-Chen/ShareGPT4V',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/TextCaps',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/refcoco',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/refcocog',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='liuhaotian/LLaVA-Instruct-150K',
...
class LLaVAPretrainPreprocessor(MessagesPreprocessor):
    ...
    def prepare_dataset(self, dataset):
        if not use_hf_hub():
            url = ('https://www.modelscope.cn/api/v1/datasets/liuhaotian/LLaVA-Pretrain/repo?'
                   'Revision=master&FilePath=images.zip')
        else:
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='liuhaotian/LLaVA-Pretrain',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/MideficsDataset',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/OK-VQA_train',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/A-OKVQA',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/OCR-VQA',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/ScienceQA',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='detection-datasets/coco',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/llava-instruct-mix-vsft',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='project-sloth/captcha-images',
        split=['train', 'validation'],
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='clevr_cogen_a_train',
...
class Voc2007MultilabelPreprocessor(ResponsePreprocessor):
    ...
    def preprocess(self, row: Dict[str, Any]) -> Dict[str, Any]:
        row['query'] = f'Multi-label classification, categories include: {list(self.CLASS_NAME)}'
