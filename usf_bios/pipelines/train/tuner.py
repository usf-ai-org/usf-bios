...
from usf_bios.optimizers.galore import GaLoreConfig, calculate_max_steps
from usf_bios.plugins import Tuner, extra_tuners
from usf_bios.tuners import USF
from usf_bios.utils import (activate_parameters, find_all_linears, find_embedding, find_norm, freeze_parameters,
                         get_logger, get_multimodal_target_regex, is_debug_mode)
...
def apply_liger(model_type: str):
    try:
        ...
        from usf_bios.model import ModelType
...
def prepare_adapter(args: SftArguments, model, *, template=None, train_dataset=None, task_type=None):
    from usf_bios.tuners import (AdaLoraConfig, AdapterConfig, BOFTConfig, LLaMAProConfig, LongLoRAModelType, LoraConfig,
                              LoRAConfig, ReftConfig, USF, VeraConfig)
    ...
    if args.train_type in ('lora', 'longlora'):
        if args.use_usf_lora:
            lora_config = LoRAConfig(lora_dtype=args.lora_dtype, **lora_kwargs)
            model = USF.prepare_model(model, lora_config)
            if is_debug_mode():
                logger.info_debug(f'lora_config: {lora_config}')
            ...
            if args.resume_from_checkpoint is None:
                ...
                if is_debug_mode():
                    logger.info_debug(f'unsloth_config: {lora_kwargs}')
        if args.train_type == 'longlora':
            assert LongLoRAModelType.LLAMA in args.model_type
            assert version.parse(transformers.__version__) >= version.parse('4.39.3')
            from usf_bios.tuners.longlora.llama import replace_llama_attn
        ...
        model = USF.prepare_model(model, adalora_config)
        if is_debug_mode():
            logger.info_debug(f'adalora_config: {adalora_config}')
        ...
        model = USF.prepare_model(model, llamapro_config)
        if is_debug_mode():
            logger.info_debug(f'llamapro_config: {llamapro_config}')
        ...
        model = USF.prepare_model(model, adapter_config)
        if is_debug_mode():
            logger.info_debug(f'adapter_config: {adapter_config}')
        ...
        model = USF.prepare_model(model, vera_config)
        if is_debug_mode():
            logger.info_debug(f'vera_config: {vera_config}')
        ...
        model = USF.prepare_model(model, boft_config)
        if is_debug_mode():
            logger.info_debug(f'boft_config: {boft_config}')
        ...
        model = USF.prepare_model(model, fourier_config)
        if is_debug_mode():
            logger.info_debug(f'fourier_config: {fourier_config}')
        ...
        if is_debug_mode():
            logger.info_debug(f'reft config: {reft_config}')
        model = USF.prepare_model(model, {'reft': reft_config})
        ...
        if is_debug_mode():
            logger.info_debug(f'bone config: {bone_config}')
        model = USF.prepare_model(model, bone_config)
    else:
