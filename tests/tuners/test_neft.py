...
from usf_bios.tuners import LoRAConfig, NEFTuneConfig, USF_BIOS
...
class TestNEFT(unittest.TestCase):
    ...
    def test_neft(self):
        model = AutoAutoModel.from_pretrained('google-bert/bert-base-uncased')
        ...
        model = USF_BIOS.prepare_model(model, config)
        model.train()
        ...
        model2 = AutoAutoModel.from_pretrained(self.tmp_dir)
        ...
        self.assertTrue(os.path.isfile(bin_file))
        model_new = AutoAutoModel.from_pretrained('google-bert/bert-base-uncased')
        model_new_2 = USF_BIOS.from_pretrained(model_new, self.tmp_dir)
    ...
    def test_neft_lora(self):
        ...
        t1 = model.embeddings.word_embeddings(inputs['input_ids'])
        model = USF_BIOS.prepare_model(model, {'c1': config, 'c2': config2})
        model.train()
        t2 = model.embeddings.word_embeddings(inputs['input_ids'])
        ...
        model_new = USF_BIOS.from_pretrained(model_new, self.tmp_dir)
        model_new.train()
