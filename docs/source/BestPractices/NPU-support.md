...
We add Ascend NPU support in usf-bios, so you can fine-tune and run inference on Ascend NPUs.
...
conda create -n usf-npu python=3.10 -y
conda activate usf-npu
...
```shell
python -c "import mindspeed.megatron_adaptor; from usf_bios.megatron.init import init_megatron_env; init_megatron_env(); print('✓ NPU environment Megatron-USF BIOS configuration verified successfully!')"
```
### Environment Viewing
...
## Fine-tuning
The following introduces the fine-tuning of LoRA. To perform full-parameter fine-tuning, simply set the parameter `--train_type full`. For **more training scripts**, refer to [here](https://github.com/us-inc/usf-bios/tree/main/examples/ascend/train).
...
Original Model:
```shell
ASCEND_RT_VISIBLE_DEVICES=0 usf infer \
    --model Qwen/Qwen2-7B-Instruct \
...
ASCEND_RT_VISIBLE_DEVICES=0 usf export --adapters xx/checkpoint-xxx --merge_lora true
...
Original model:
```shell
ASCEND_RT_VISIBLE_DEVICES=0 usf deploy --model Qwen/Qwen2-7B-Instruct --max_new_tokens 2048
```

After LoRA fine-tuning:
```shell
ASCEND_RT_VISIBLE_DEVICES=0 usf deploy --adapters xxx/checkpoint-xxx --max_new_tokens 2048

# Merge LoRA and deploy
ASCEND_RT_VISIBLE_DEVICES=0 usf export --adapters xx/checkpoint-xxx --merge_lora true
ASCEND_RT_VISIBLE_DEVICES=0 usf deploy --model xxx/checkpoint-xxx-merged --max_new_tokens 2048
```
...
# Merge LoRA and deploy
ASCEND_RT_VISIBLE_DEVICES=0 usf export \
