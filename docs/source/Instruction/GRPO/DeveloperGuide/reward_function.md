...
```python
from usf_bios.plugins import ORM, orms
...
You can add the reward function in [plugin program](https://github.com/us-inc/usf-bios/blob/main/examples/train/grpo/plugin/plugin.py), register it using the parameter `--external_plugins examples/train/grpo/plugin/plugin.py`, and specify it via the `reward_funcs` parameter.

For execution scripts, refer to [here](https://github.com/us-inc/usf-bios/tree/main/examples/train/grpo/plugin/run_external_reward_func.sh).

## Async Reward Functions

**Version requirement**: usf-bios>=3.12.1
...
```python
from usf_bios.plugins import AsyncORM, orms
import asyncio
...
USF BIOS supports using both synchronous and asynchronous reward functions simultaneously. The trainer automatically detects the type of reward function:
...
The [plugin](https://github.com/us-inc/usf-bios/blob/main/examples/train/grpo/plugin/plugin.py) file provides an example of a generative reward model (async_genrm) that calls the `usf deploy` service.

## Built-in Reward Functions
USF BIOS includes five rule-based reward functions (code can be found in usf/plugins/orm.py).
