...
from usf_bios.dataset import DatasetSyntax, sample_dataset
from usf_bios.plugins import extra_tuners
from usf_bios.template import update_generation_config_eos_token
...
def _select_dataset(dataset, max_length):
    ...
    if len(new_dataset) < len(dataset):
        logger.info_debug(f'Dataset filtered, origin length: {len(dataset)}, filtered dataset length: {len(new_dataset)}')
    return new_dataset
