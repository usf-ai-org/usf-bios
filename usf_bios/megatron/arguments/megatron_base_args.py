...
from usf_bios.megatron.model import get_megatron_model_meta
from usf_bios.megatron.utils import convert_hf_config
...
class MegatronBaseArguments(MegatronArguments, BaseArguments):
    ...
    def __post_init__(self):
        ...
        if self.streaming:
            ...
            if self.num_workers > 1:
                self.num_workers = 1
                logger.info_debug('Using streaming dataset, setting args.num_workers to 1.')
    ...
    def init_model_args(self, tokenizer, config):
        if self.task_type == 'seq_cls':
            ...
            if is_debug_mode():
                logger.info_debug(f'args.problem_type: {self.problem_type}')
