...
from usf_bios.model import MODEL_MAPPING, get_model_info_meta, get_model_name
from usf_bios.utils import HfConfigFactory, get_dist_setting, get_logger, json_parse_to_dict
...
class ModelArguments:
    ...
    Args:
        ...
        model_type (Optional[str]): The model type. In usf-bios, a 'model_type' groups models with the same
            ...
            in usf-bios differs from the `model_type` in `config.json`. Custom models usually require registering
        ...
        rope_scaling (Optional[str]): The RoPE scaling type. You can pass a string like 'linear', 'dynamic', or
            'yarn', and usf-bios will automatically set the corresponding `rope_scaling` and override the
    ...
    def _init_rope_scaling(self):
        ...
        if 'factor' not in rope_scaling and self.max_model_len is None and rope_type == 'default':
            ...
            logger.info_debug(f'Setting args.rope_scaling: {rope_scaling}')
            return
        ...
        self.rope_scaling = rope_scaling
        logger.info_debug(f'Setting args.rope_scaling: {rope_scaling}')
        logger.info_debug(f'Setting args.max_model_len: {self.max_model_len}')
