...
from usf_bios.dataset import (AddLengthPreprocessor, DatasetLoader, EncodePreprocessor, IterablePackingDataset,
                           LazyLLMDataset, PackingDataset, load_dataset)
from usf_bios.infer_engine import prepare_generation_config
from usf_bios.plugins import EarlyStopCallback, extra_callbacks
from usf_bios.ray import RayHelper
from usf_bios.sequence_parallel import sequence_parallel
from usf_bios.trainers import TrainerFactory
from usf_bios.utils import (append_to_jsonl, get_logger, get_model_parameter_info, is_debug_mode, is_master,
                         plot_images, setup_graceful_exit, show_dataset_summary, show_model_info,
                         show_startup_banner, show_success, show_training_complete, show_training_failed,
                         start_training_timer, stat_array)
...
class USFSft(USFPipeline, TunerMixin):
    ...
    def _prepare_generation_config(self):
        ...
        if is_debug_mode():
            logger.info_debug(f'model.generation_config: {self.model.generation_config}')
    ...
    def _prepare_model_tokenizer(self, **kwargs):
        ...
        if is_debug_mode():
            if hasattr(self.model, 'hf_device_map'):
                if is_debug_mode():
                    logger.info_debug(f'model.hf_device_map: {self.model.hf_device_map}')
            logger.info_debug(f'model_info: {self.model.model_info}')
    ...
    def _get_dataset(self):
        ...
        if args.truncation_strategy == 'split':
            logger.info_debug(f'train_dataset: {train_dataset}')
            logger.info_debug(f'val_dataset: {val_dataset}')
    ...
    def _save_val_dataset(self, val_dataset):
        ...
        if is_master() and isinstance(val_dataset, HfDataset) and not args.val_dataset:
            ...
            logger.info_debug(f'The split dataset from the training set will be saved at: {val_dataset_path}.')
    ...
    def run(self):
        ...
        if args.task_type == 'seq_cls':
            ...
            if is_debug_mode():
                logger.info_debug(f'args.problem_type: {args.problem_type}')
        args.save_args()
        ...
        if is_debug_mode():
            ...
            if is_debug_mode():
                logger.info_debug(f'model_parameter_info: {model_parameter_info}')
    ...
    def _save_trainer_state(self, trainer):
        ...
        logger.info_debug(f'last_model_checkpoint: {state.last_model_checkpoint}')
        logger.info_debug(f'best_model_checkpoint: {state.best_model_checkpoint}')

        # Visualization
        if is_master():
            if 'tensorboard' in training_args.report_to:
                images_dir = os.path.join(training_args.output_dir, 'images')
                logger.info_debug(f'images_dir: {images_dir}')
    ...
    def train(self, trainer):
        ...
        logger.info_debug(f'The logging file will be saved in: {logging_path}')
        try:
    ...
    def _prepare_callbacks(self):
        ...
        if args.early_stop_interval is not None and args.early_stop_interval > 0:
            ...
            logger.info_debug('You are using the default early stop callback, this is a implementation of '
    ...
    def _stat_dataset(dataset: Union[HfDataset, PackingDataset, LazyLLMDataset]):
        ...
        logger.info_debug(f'Dataset Token Length: {stat_str}')
        return stat_str
