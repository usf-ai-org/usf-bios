...
def infer_multilora(infer_request: 'InferRequest', infer_backend: Literal['vllm', 'transformers']):
    # Dynamic LoRA
    adapter_path = safe_snapshot_download('usf_bios/test_lora')
    adapter_path2 = safe_snapshot_download('usf_bios/test_lora2')
...
def infer_lora(infer_request: 'InferRequest'):
    ...
    # model = USF.from_pretrained(model, adapter_path)
...
if __name__ == '__main__':
    from usf_bios import (TransformersEngine, RequestConfig, AdapterRequest, InferRequest, BaseArguments,
                       get_model_processor, safe_snapshot_download, USF, get_template)
