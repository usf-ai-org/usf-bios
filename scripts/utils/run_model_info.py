...
from usf_bios.model import MODEL_MAPPING, ModelType
from usf_bios.template import TEMPLATE_MAPPING, TemplateType
from usf_bios.utils import is_megatron_available
...
def get_model_info_table():
 ...
 end_words = [['### multimodal large models', '## '], ['### Multimodal large models', '## Datasets']]
 result = [
 ...
 from usf_bios.megatron import model
 ...
 res_llm.append(r)
 print(f'LLM: {len(res_llm)}, MLLM: {len(res_mllm)}')
 print(f'[Megatron] LLM: {mg_count_llm}, MLLM: {mg_count_mllm}')
 text = ['', ''] # llm, mllm
