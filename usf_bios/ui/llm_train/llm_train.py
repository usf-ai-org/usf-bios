...
from usf_bios.arguments import ExportArguments, RLHFArguments, get_supported_tuners
        ...
        if sys.platform == 'win32':
            ...
            if envs:
                ...
                for env in envs:
                    _envs += f'set {env} && '
                envs = _envs
            run_command = f'{cuda_param}{ddp_param}{envs}start /b usf sft {params} > {log_file} 2>&1'
        else:
            run_command = f'{cuda_param} {ddp_param} {envs} nohup usf {cmd} {params} > {log_file} 2>&1 &'
        logger.info_debug(f'Run training: {run_command}')
        if model:
            record = {}
            for key, value in zip(keys, args):
