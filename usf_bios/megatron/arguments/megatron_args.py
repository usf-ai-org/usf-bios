...
from usf_bios.arguments import ModelArguments
from usf_bios.model import get_model_info_meta
from usf_bios.utils import get_dist_setting, get_logger, json_parse_to_dict
...
class RLHFMegatronArgumentsMixin:
    ...
    # see details in usf_bios/plugins/orm.py
...
class MegatronArguments(ExtraMegatronArguments):
    ...
    wandb_project: str = 'megatron-usf'
    ...
    def __post_init__(self):
        ...
        if self.adapters or self.adapter_load or self.ref_adapter_load:
            if self.train_type == 'full':
                ...
                logger.info_debug('Setting args.train_type: lora')
        if self.adapters:
    ...
    def _init_no_rope_fusion(self):
        ...
        logger.info_debug(f'Setting args.no_rope_fusion: {self.no_rope_fusion}.')
    ...
    def _load_adapter_config(self):
        ...
        for k, v in adapter_config.items():
            ...
            if v != getattr(self, k):
                setattr(self, k, v)
                logger.info_debug(f'Setting {k}: {v}')
