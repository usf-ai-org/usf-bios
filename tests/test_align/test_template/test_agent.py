...
def test_react_en():
    ...
    dataset = load_dataset('glaiveai/glaive-function-calling-v2')[0]
    data = dataset[6]
    data['messages'].insert(1, data['messages'][1])
    data['messages'].insert(3, data['messages'][3])
    template.template_backend = 'usf_bios'
...
if __name__ == '__main__':
    from usf_bios import load_dataset, TransformersEngine, InferRequest, RequestConfig, agent_template_map
    # test_react_en()
    # test_react_zh()
    # test_qwen_en()
    # test_qwen_zh()
