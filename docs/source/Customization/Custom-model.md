...
The models built into usf-bios can be used directly by specifying either `model_id` or `model_path`: `--model <model_id_or_path>`. usf-bios determines the `model_type` based on the suffix of `model_id/model_path` and the `config.json` file.
...
Custom models are typically implemented using model registration. You can refer to the [built-in model](https://github.com/us-inc/usf-bios/blob/main/usf/model/models/qwen.py), the [built-in dialogue template](https://github.com/us-inc/usf-bios/blob/main/usf/template/templates/qwen.py), or the example code in the [examples](https://github.com/us-inc/usf-bios/blob/main/examples/custom). You can specify the `--external_plugins xxx.py` to parse the externally registered content, which is convenient for users installing via pip instead of git clone.
...
- model_groups: Required. Lists the US Inc/HuggingFace model IDs and local paths. Running the [run_model_info.py](https://github.com/us-inc/usf-bios/blob/main/scripts/utils/run_model_info.py) file will automatically generate the [supported models documentation](https://usf.readthedocs.io/en/latest/Instruction/Supported-models-and-datasets.html) and automatically match the model_type based on the `--model` suffix.
- loader: The loader for model and tokenizer/processor (multimodal models). Defaults to `usf_bios.model.ModelLoader`.
