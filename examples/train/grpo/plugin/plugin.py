...
from usf_bios.infer_engine import RequestConfig, TransformersEngine
from usf_bios.infer_engine.protocol import ChatCompletionResponse, ChatCompletionResponseChoice, RolloutInferRequest
from usf_bios.plugins import ORM, AsyncORM, orms, rm_plugins
# register context manager(used in gym training)
from usf_bios.plugins.context_manager import ContextManager, context_managers
from usf_bios.plugins.env import Env, envs
from usf_bios.plugins.multi_turn import MultiTurnScheduler, multi_turns
from usf_bios.plugins.rm_plugin import DefaultRMPlugin
...
# For additional reward functions, refer to usf_bios/plugins/orm.py.
...
class MultiTurnThinkingTips(ORM):
    ...
    def __init__(self):
        from usf_bios.plugins.orm import MathAccuracy
...
class AsyncGenRMReward(AsyncORM):
    ...
    Usage:
        1. Deploy a reward model using usf deploy:
           ```bash
           usf deploy --model Qwen/Qwen2.5-7B-Instruct --port 8000 --infer_backend vllm
           ```
    ...
    def __init__(self):
        ...
        try:
            ...
            raise RuntimeError('Failed to connect to the model service. Please deploy the model '
                               "using 'usf deploy --model <model_name> --port 8000 --infer_backend vllm'.") from e
...
For GenRM you can refer to usf_bios/plugins/rm_plugin/GenRMPlugin
"""
