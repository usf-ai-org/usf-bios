...
Pluginization is a significant new feature introduced in USF BIOS 3.0. We aim to make the customization of the development process more natural for developers through a plugin-based approach.

## Callback Mechanism

An example can be found [here](https://github.com/us-inc/usf-bios/blob/main/usf/plugins/callback.py).
...
Here, we place more emphasis on the words "" (mathematics) and "" (important) by increasing their weights to 2.0.
...
An example can be found [here](https://github.com/us-inc/usf-bios/blob/main/usf/metrics).
...
An example can be found [here](https://github.com/us-inc/usf-bios/blob/main/usf/optimizers/mapping.py).
- Apply different learning rates to different parts of the model. For example, use separate learning rates for ViT and LLM, as referenced [here](https://github.com/us-inc/usf-bios/blob/main/examples/train/multimodal/lora_llm_full_vit/custom_plugin.py).
...
The example is [here](https://github.com/us-inc/usf-bios/blob/main/usf/agent_template/mapping.py).

## Customizing Tuners

An example can be found [here](https://github.com/us-inc/usf-bios/blob/main/usf/plugins/tuner.py).
- For the multimodal model, full-parameter training is applied to the ViT part, while LoRA training is used for the LLM part. Refer to [here](https://github.com/us-inc/usf-bios/tree/main/examples/train/multimodal/lora_llm_full_vit).
- For Phi4-multimodal, train its existing LoRA directly without adding extra LoRA. Refer to [here](https://github.com/us-inc/usf-bios/blob/main/examples/train/plugins/tuner_phi4_mm.sh).

Tuner customization is another unique feature of USF BIOS. Developers can bypass the complex tuner initialization process and code integration costs by registering new tuners here:
...
These three methods are invoked during the USF BIOS training process, allowing developers to use their tuners without reading the complex training code.

## PRM (Process Reward Model)

An example can be found [here](https://github.com/us-inc/usf-bios/blob/main/usf/plugins/prm.py).

PRM stands for Process Reward Model, which is used in the `usf sample` command. PRM needs to support simple interfaces:
...
The InferRequest comes from `usf_bios.infer_engine`, and the returned `List[Union[float, List[float]]]` may contain a reward or several rewards. Developers can access queries and responses in infer_requests and split them according to their own methods, for example:
...
An example can be found [here](https://github.com/us-inc/usf-bios/blob/main/usf/plugins/orm.py).
