...
USF BIOS supports eval (evaluation) capabilities to provide standardized evaluation metrics for both raw models and trained models.

## Capability Introduction

USF BIOS's eval capability utilizes the EvalScope evaluation framework from the Magic Tower community, which has been advanced in its encapsulation to support the evaluation needs of various models.
...
```
Where:
- model: Can specify a local model path or a model ID on huggingface
...
USF BIOS supports using EvalScope to evaluate the current model during the training process, allowing for timely understanding of the model's training effectiveness.
...
More evaluation examples can be found in [examples](https://github.com/us-inc/usf-bios/tree/main/examples/eval).
