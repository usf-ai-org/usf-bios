...
from usf_bios.arguments import InferArguments
...
class USFInfer(USFPipeline):
    ...
    def __init__(self, args: Optional[Union[List[str], InferArguments]] = None) -> None:
        ...
        if args.infer_backend == 'transformers':
            ...
            if is_debug_mode():
                logger.info_debug(f'model: {self.infer_engine.model}')
        else:
    ...
    def get_infer_engine(args: InferArguments, template=None, **kwargs):
        ...
        if infer_backend == 'transformers':
            ...
            from usf_bios.infer_engine import SglangEngine
    ...
    def run(self) -> List[Dict[str, Any]]:
        ...
        if args.result_path:
            logger.info_debug(f'The inference results have been saved to result_path: `{args.result_path}`.')
        return result
    ...
    def infer_cli(self) -> List[Dict[str, Any]]:
        ...
        if is_debug_mode():
            logger.info_debug(f'request_config: {request_config}')
        ...
        if support_multi_round:
            ...
            logger.info_debug('The current template only supports single-round dialogues.')
    ...
    def infer_dataset(self) -> List[Dict[str, Any]]:
        ...
        if request_config and request_config.stream:
            ...
            if args.write_batch_size < len(val_dataset) and args.result_path:
                if is_debug_mode():
                    logger.info_debug(f'args.result_path: {args.result_path}')
            prog_bar = tqdm(
