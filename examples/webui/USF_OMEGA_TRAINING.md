# USF Omega Training WebUI

A dedicated fine-tuning interface for **arpitsh018/usf-omega-40b-base**.
...
|------|---------------|-------|---------|----------|
| **LoRA** | ~80GB | Fast | Excellent | Recommended for most cases |
| **QLoRA** | ~40GB | Medium | Very Good | Limited VRAM |
| **Full** | ~160GB+ | Slow | Best | Maximum quality, multi-GPU |

---

## Default Hyperparameters

Based on [LoRA best practices](https://thinkingmachines.ai/blog/lora/):
...
| LoRA Alpha (α) | 32 | Standard practice |
...
### QLoRA Defaults
Same as LoRA with 4-bit quantization (BNB).
...
```bash
PYTHONPATH=/workspace/usf-usf-bios-prod python usf/cli/sft.py \
    ...
    --max_length 4096 \
    --output_dir "output/usf-omega-sft" \
    --lora_rank 64 \
    --lora_alpha 32 \
    --lora_dropout 0.05 \
