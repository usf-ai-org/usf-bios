...
from usf_bios.template import split_str_parts_by
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='tatsu-lab/alpaca',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='Yukang/LongAlpaca-12k',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='LooksJuicy/ruozhiba',
...
register_dataset(
    DatasetMeta(ms_dataset_id='math-trn-format', preprocess_func=MathTrnPreprocessor(), tags=['math']))
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='firefly-train-1.1M',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='synthetic_text_to_sql',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='sql-create-context',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='tigerbot-law-plugin',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='greengerong/leetcode-solutions-python',
...
class MultiRoleAgentPreprocessor(RowPreprocessor):
    ...
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        res_prompt = '\n\n[Notes]\n1. This is a chat room, do not send private messages to anyone\n2. Speak only for yourself, do not impersonate others, only reply based on chat history\n3. Keep it brief, do not say too much, no more than 50 words '
        history_prompt = '\n\n【chat history】'
        conv_prompt = '\n {name}:{content}'
        ...
        if conv[0]['from'] == 'user':
            ...
            if '[Notes]' not in system and system:
                system += res_prompt
...
register_dataset(DatasetMeta(ms_dataset_id='usf_bios/ToolBench', tags=['chat', 'agent', 'multi-round']))
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='Anthropic/hh-rlhf',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='Anthropic/hh-rlhf-cn',
...
register_dataset(
    DatasetMeta(ms_dataset_id='argilla/ultrafeedback-binarized-preferences-cleaned', tags=['rlhf', 'kto']))
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='glaiveai/glaive-function-calling-v2',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='databricks-dolly-15k',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='mlabonne/orpo-dpo-mix-40k',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/sharegpt',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='usf_bios/self-cognition',
