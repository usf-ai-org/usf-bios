...
GRPOTrainer underwent a code refactoring in usf-bios3.5. If you are using a usf version < 3.5, please refer to the [stable documentation](https://github.com/us-inc/usf-bios/blob/v3.4.1/docs/source/Instruction/GRPO.md).
...
For training script examples, refer to [examples](https://github.com/us-inc/usf-bios/tree/main/examples/train/grpo).
...
Use the `usf rollout` command to deploy the vLLM server (currently only supports vLLM backend):
```bash
...
USF BIOS 3.10 optimizes weight synchronization, and setting the following parameters can further improve the weight synchronization speed for LoRA training:
...
For implementation details, please refer to the [PR](https://github.com/us-inc/usf-bios/pull/5773)
...
Training-inference consistency metrics, prefixed with rollout_correction (usf-bios>=3.11), requires setting `log_rollout_offpolicy_metrics=true` or `rollout_importance_sampling_mode`:
...
Refer to [issue](https://github.com/us-inc/usf-bios/issues/3912).
...
When `val_dataset` is not explicitly passed, the `split_dataset_ratio` parameter is responsible for splitting part of the `dataset` into a validation dataset, which defaults to splitting 1% of the data. (In "usf-bios>=3.6", the default value of split_dataset_ratio will be changed from 0.01 to 0.)
...
**8. Difference between usf deploy and usf rollout**

- usf deploy is primarily used for model deployment and inference. It supports various engines such as PT, vLLM, and SGLang, and is compatible with streaming inference as well as the OpenAI API format.

- usf rollout, on the other hand, is dedicated to GRPO rollout acceleration. Currently, it only supports the vLLM engine and comes with built-in automatic weight synchronization.
