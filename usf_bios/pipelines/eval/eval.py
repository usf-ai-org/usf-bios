...
from usf_bios.dataset import MediaResource
from usf_bios.utils import append_to_jsonl, get_logger
...
class USFEval(USFPipeline):
    ...
    def run(self):
        ...
        if args.result_jsonl:
            ...
            logger.info_debug(f'The eval result have been saved to result_jsonl: `{args.result_jsonl}`.')
        return eval_report
                ...
                if os.path.exists('data'):
                    if not os.path.exists(os.path.join('data', 'CMB')):
                        raise RuntimeError('Opencompass need a `data` folder in your work dir('
                                           'which will be created automatically by usf eval), '
