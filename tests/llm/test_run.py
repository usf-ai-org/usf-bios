...
from usf_bios import (InferArguments, RLHFArguments, SftArguments, Trainer, TrainingArguments, get_logger, infer_main,
                   rlhf_main, sft_main)
...
class TestRun(unittest.TestCase):
    ...
    def test_hf_hub(self):
        ...
        train_dataset_fnames = [
            'alpaca.csv', 'chatml.jsonl', 'usf_bios_pre.jsonl', 'usf_bios_single.csv', 'usf_bios_multi.jsonl',
            'usf_bios_multi.json#2'
        ]
    ...
    def test_vl_audio(self):
        ...
        dataset_list = [
            'detection-datasets/coco:validation#100', 'speech_asr/speech_asr_aishell1_trainsets:validation#100'
        ]
    ...
    def test_custom_dataset(self):
        ...
        train_dataset_fnames = [
            'alpaca.csv', 'chatml.jsonl', 'usf_bios_pre.jsonl', 'usf_bios_single.csv', 'usf_bios_multi.jsonl',
            'usf_bios_multi.json', 'sharegpt.jsonl'
        ]
        val_dataset_fnames = [
            'alpaca.jsonl',
            'alpaca2.csv',
        ...
        for num_train_epochs in [1, 2]:
            sft_args = SftArguments(
                ...
                dataset=['usf_bios/self-cognition#20'] + [os.path.join(folder, fname) for fname in train_dataset_fnames],
    ...
    def test_rlhf(self):
        ...
        for rlhf_type in rlhf_types:
            dataset = ('Anthropic/hh-rlhf:harmless_base_cn#100'
                       if rlhf_type != 'kto' else 'argilla/ultrafeedback-binarized-preferences-cleaned#100')
            train_kwargs = {}
            if rlhf_type == 'ppo':
        ...
        for rlhf_type in visual_rlhf_types:
            for model in test_model:
                dataset_name = 'usf_bios/RLAIF-V-Dataset#100'
                output = rlhf_main(
                    RLHFArguments(
                        rlhf_type=rlhf_type,
                        model=model,
    ...
    def test_loss_matching(self):
        ...
        if not __name__ == '__main__':
            ...
            return
        losses = []
        for use_usf_bios_lora in [False, True]:
            bool_var = use_usf_bios_lora
            torch.cuda.empty_cache()
            output = sft_main([
                '--model', 'Qwen/Qwen-7B-Chat', '--save_steps', '5', '--dataset',
                'greengerong/leetcode-solutions-python#200', '--output_dir', output_dir, '--gradient_checkpointing',
                'true', '--max_new_tokens', '100', '--attn_impl', 'flash_attn', '--target_modules', 'all-linear',
                '--seed', '0', '--lora_bias', 'all', '--modules_to_save', 'lm_head', '--use_usf_bios_lora',
                str(use_usf_bios_lora), '--num_train_epochs', '1', '--gradient_accumulation_steps', '16'
            ])
            best_model_checkpoint = output['best_model_checkpoint']
            print(f'best_model_checkpoint: {best_model_checkpoint}')
        ...
        print(f'usf_bios_loss: {losses[0]}')
    ...
    def test_pai_compat(self):
        if not __name__ == '__main__':
            # ignore citest error in github
            return
        from usf_bios import sft_main, infer_main
...
class TestTrainer(unittest.TestCase):
    ...
    def test_trainer(self):
        ...
        model = AutoModel.from_pretrained(model_dir, task='text-classification', num_labels=num_labels)
