...
from usf_bios.utils import find_free_port, get_device_count, get_logger
...
class ExpManager:
    ...
    def _build_eval_cmd(self, exp: Experiment):
        ...
        if best_model_checkpoint is not None:
            if not os.path.exists(os.path.join(best_model_checkpoint, 'args.json')):
                cmd = f'usf eval --ckpt_dir {best_model_checkpoint} ' \
                      + f'--infer_backend transformers --train_type full --eval_dataset {" ".join(eval_dataset)}'
        else:
            cmd = f'usf eval --model {exp.args.get("model")} --infer_backend transformers ' \
    ...
    def _build_cmd(self, exp: Experiment):
        ...
        if exp.cmd == 'sft':
            ...
            from usf_bios import RLHFArguments
            args = exp.args
