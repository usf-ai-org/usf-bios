...
from usf_bios.arguments import BaseArguments, RLHFArguments
from usf_bios.dataset import DatasetLoader, load_dataset
...
class USFRLHF(USFSft):
    ...
    def _prepare_single_model(self, key, origin_key, model_type, model_revision):
        ...
        if origin_key in {'ref', 'reward', 'teacher'}:
            ...
            if is_debug_mode():
                logger.info_debug(f'value_model: {model}')
                if is_debug_mode():
                    logger.info_debug(f'value_model_parameter_info: {model_parameter_info}')
