...
Sampling is one of the newly supported key capabilities of USF BIOS. This feature can be understood as the practical implementation of `test-time compute`. Additionally, this capability is crucial for the implementation of RFT (Reinforcement Fine-Tuning).

## Capability Introduction

The sampling capability of USF BIOS can be demonstrated with the following example:

```shell
usf sample --model LLM-Research/Meta-Llama-3.1-8B-Instruct --sampler_engine transformers --num_return_sequences 5 --dataset tatsu-lab/alpaca#5
```
...
```shell
pip install usf-bios[llm] -U
```

Or install usf from source:
...
```shell
usf sample --model LLM-Research/Meta-Llama-3.1-8B-Instruct --sampler_engine lmdeploy --num_return_sequences 5 --n_best_to_keep 2 --dataset tastelikefeet/competition_math#5 --prm_model GRM-llama3.2-3B-rewardmodel-ft --orm_model math
```
...
Please refer to the [Reinforcement Fine-Tuning Script](https://github.com/us-inc/usf-bios/tree/main/examples/train/rft/rft.py). This script provides a practical example of using sampling for reinforcement fine-tuning.
...
USF BIOS's sample supports using the OpenAI API to distill data with large models. Example:
