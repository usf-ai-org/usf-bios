...
from usf_bios.utils import TB_COLOR, TB_COLOR_SMOOTH, format_time, get_logger, read_tensorboard_file, tensorboard_smoothing
...
class Runtime(BaseUI):
    ...
    locale_dict = {
        ...
        'running_tasks': {
            ...
            'info': {
                'en': 'All running tasks(started by usf sft/pt)'
            }
        },
        'refresh_tasks': {
            'value': {
    ...
    def get_plot(cls, task):
        if not task or 'usf sft' in task or 'usf pt' in task:
            return cls.sft_plot
    ...
    def start_tb(cls, logging_dir):
        ...
        logger.info_debug('===========Tensorboard Log============')
        logger.info_debug('\n'.join(lines))
    ...
    def refresh_tasks(running_task=None, group=None):
        ...
        process_name = 'usf_bios'
        negative_names = ['usf_bios.exe', 'usf-script.py']
    ...
    def parse_info_from_cmdline(task):
        ...
        if '/cmd:' in task:
            for i in range(3):
                ...
                task = task[slash + 1:]
        if 'usf sft' in task:
            args = task.split('usf sft')[1]
        elif 'usf pt' in task:
            args = task.split('usf pt')[1]
        elif 'usf rlhf' in task:
            args = task.split('usf rlhf')[1]
        else:
    ...
    def cmd_to_sh_format(cmd):
        ...
        cmd_sh += (env_params + ' \\\n')
        usf_cmd = params[0].split('nohup')[1].strip()
        cmd_sh += ('nohup ' + usf_cmd + ' \\\n')
