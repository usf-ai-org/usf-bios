...
from usf_bios.utils import (Processor, get_device_count, get_dist_setting, get_env_args, get_logger, is_deepspeed_enabled,
                         to_device)
...
class QwenLoader(ModelLoader):
    ...
    def get_model(self, model_dir: str, config, processor, model_kwargs) -> PreTrainedModel:
        ...
        try:
            ...
            logger.info_debug('registered_causal_mask to cuda')
...
register_model(
    ModelMeta(
        ...
        [ModelGroup([
            Model('iic/US Inc-Agent-7B'),
            Model('iic/US Inc-Agent-14B'),
        ])],
        QwenLoader,
        ...
        [
            ModelGroup(
                [
                    ...
                    # awq
                    Model('usf_bios/Qwen3-30B-A3B-AWQ', 'cognitivecomputations/Qwen3-30B-A3B-AWQ'),
                    Model('usf_bios/Qwen3-235B-A22B-AWQ', 'cognitivecomputations/Qwen3-235B-A22B-AWQ'),
                ],
                TemplateType.qwen3),
            ModelGroup([
            ...
            ModelGroup(
                [
                    ...
                    # awq
                    Model('usf_bios/Qwen3-235B-A22B-Instruct-2507-AWQ'),
                ],
                TemplateType.qwen3_nothinking),
            ModelGroup([
                ...
                Model('usf_bios/Qwen3-Coder-480B-A35B-Instruct-AWQ'),
            ],
            ...
            ModelGroup(
                [
                    ...
                    # awq
                    Model('usf_bios/Qwen3-235B-A22B-Thinking-2507-AWQ'),
                ],
                TemplateType.qwen3_thinking),
        ],
        requires=['transformers>=4.51'],
    ...
    if read_backend == 'torchvision':
        ...
        def _new_read_video(ele: dict):
            try:
                ...
                from usf_bios.template import load_file  # base64
                ele['video'] = load_file(ele['video'])
                return read_video_func(ele)
    else:

        def _new_read_video(ele: dict):
            from usf_bios.template import load_file
            ele['video'] = load_file(ele['video'])
            return read_video_func(ele)
