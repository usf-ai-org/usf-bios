...
from usf_bios.utils import is_debug_mode, HfConfigFactory, get_logger, to_abspath
...
class MegatronExportArguments(MegatronBaseArguments):
    ...
    def _init_save(self):
        ...
        if is_debug_mode():
            logger.info_debug(f'args.save: `{self.save}`')
    ...
    def __post_init__(self):
        ...
        if self.to_hf or self.to_mcore:
            ...
            if self.model_info.is_moe_model is not None and self.tensor_model_parallel_size > 1:
                ...
                logger.info_debug('Settting args.sequence_parallel: True')
