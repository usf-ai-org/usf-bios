...
from usf_bios.utils import (activate_parameters, deep_getattr, find_layers, freeze_parameters, get_logger,
                         get_model_parameter_info, is_debug_mode)
...
def prepare_adapter(model):
    from usf_bios.tuners import LoraConfig, USF
    args = get_args()
...
def prepare_mcore_model(model):
    ...
    if is_debug_mode():
        logger.info_debug(f'model: {model}')
    logger.info_if(
