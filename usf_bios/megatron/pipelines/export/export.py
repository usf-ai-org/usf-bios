...
from usf_bios.megatron.arguments import MegatronExportArguments
from usf_bios.megatron.convert import test_convert_precision
from usf_bios.megatron.utils import adapter_state_dict_context, patch_load_base_checkpoint, prepare_mcore_model
from usf_bios.pipelines import USFPipeline, prepare_model_template
from usf_bios.utils import is_debug_mode, disable_safe_ddp_context_use_barrier, get_logger, is_last_rank

logger = get_logger()


class MegatronExport(USFPipeline):
    ...
    def convert_mcore2hf(self) -> None:
        ...
        logger.info_debug('Converting weights and saving the model...')
        ...
        if is_debug_mode():
            logger.info_debug(f'Successfully saved HF model weights in `{args.save}`.')
    ...
    def convert_hf2mcore(self) -> None:
        ...
        post_process = mpu.is_pipeline_last_stage()
        mg_model = megatron_model_meta.model_provider(pre_process=pre_process, post_process=post_process)
        logger.info_debug('Megatron model created successfully.')
        bridge = megatron_model_meta.bridge_cls()
        ...
        if args.adapters or args.adapter_load is not None:
            ...
            if args.merge_lora:
                logger.info_debug('Merge LoRA...')
                mg_model = peft_model.merge_and_unload()
        logger.info_debug('Successfully transferred HF model weights to MG model.')
        _test_convert_precision = strtobool(os.getenv('USF_TEST_CONVERT_PRECISION', '0'))
        if not _test_convert_precision:
            args.save_args(args.save)
            logger.info_debug('Saving the model...')
            save_peft_format = args.train_type == 'lora' and not args.merge_lora
