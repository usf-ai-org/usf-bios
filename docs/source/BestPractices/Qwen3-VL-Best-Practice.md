...
pip install "usf-bios>=4.0"
# pip install "vllm>=0.11.0"  # If using the vLLM inference backend for inference
```
- About slow training: When using PyTorch 2.9, you may encounter slow training issues with the conv3d operator. Please try using PyTorch 2.8 as a workaround. For more information, refer to [this issue](https://github.com/pytorch/pytorch/issues/166122). In usf-bios>=3.11.2, you can work around this issue by setting `USF BIOS_PATCH_CONV3D=1`. For more details, see [this issue](https://github.com/us-inc/usf-bios/issues/7108).
...
messages = [
    {
        ...
        "content": [
            {
                "type": "video",
                "video": "https://placehold.co/400x300/mp4?text=video",
...
Inference using usf-bios's TransformersEngine:

```python
import os
# os.environ['USF BIOS_DEBUG'] = '1'
os.environ['CUDA_VISIBLE_DEVICES'] = '0'
os.environ['VIDEO_MAX_TOKEN_NUM'] = '128'
os.environ['FPS_MAX_FRAMES'] = '16'

from usf_bios.infer_engine import TransformersEngine, InferRequest, RequestConfig
...
## Training
This section introduces how to train Qwen3-VL using usf-bios and Megatron-USF BIOS. We recommend using usf-bios (i.e., transformers backend, which is more convenient and simple) for Dense models, and Megatron-USF BIOS (i.e., megatron backend, which offers faster training speed; see benchmark [here](../Megatron-USF BIOS/Quick-start.md#benchmark)) for MoE models.
...
Qwen3-VL's bbox output uses normalized 1000 relative coordinates. You can use the grounding dataset format provided by usf-bios, where the coordinates in "bbox" are absolute coordinates, and usf-bios will automatically convert absolute coordinates to normalized 1000 relative coordinates. For more information, please refer to the [Grounding Dataset Format Documentation](../Customization/Custom-dataset.md#grounding).
...
- If you find the preprocessing time too long, you can remove `--packing`, or use [cached dataset](https://github.com/us-inc/usf-bios/tree/main/examples/train/cached_dataset).
...
usf sft \
    --model Qwen/Qwen3-VL-4B-Instruct \
    --dataset 'tatsu-lab/alpaca#10000' \
              'linxy/LaTeX_OCR:human_handwrite#5000' \
              'usf_bios/VideoChatGPT:Generic#2000' \
...
### MoE Models
Below is a fine-tuning script for the `Qwen3-VL-30B-A3B-Instruct` model. We use Megatron-USF BIOS for single-machine full-parameter training. We still use mixed data for training; this example script has no practical value. Training requires 8 * 80GiB GPU memory, and training time is 20 minutes.

For Megatron-USF BIOS environment installation, please refer to the [Megatron-USF BIOS Documentation](../Megatron-USF BIOS/Quick-start.md). Megatron-USF BIOS shares the template and dataset modules with usf-bios, so the custom dataset format and model-specific environment variables introduced earlier still apply.

The fine-tuning script is as follows. For adjusting training techniques and parallelism strategies, refer to the [Megatron-USF BIOS Documentation](../Megatron-USF BIOS/Quick-start.md#training-tips).
...
usf infer \
    ...
    --load_data_args true
```
Use the following command to push the trained weights to US Inc:
