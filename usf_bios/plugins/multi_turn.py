...
from usf_bios.infer_engine import GRPOVllmEngine
from usf_bios.infer_engine.protocol import (ChatCompletionResponse, ChatCompletionResponseChoice, RequestConfig,
...
class MultiTurnScheduler(RolloutScheduler, ABC):
    ...
    Loss mask configuration:
        ...
        1. Use the built-in `loss_scale` parameter in usf-bios and do not return response token ids.
    ...
    async def run(self, infer_request: 'RolloutInferRequest', request_config: 'RequestConfig',
        ...
        Important Notes:
            - Method overriding is only supported when using server mode (usf rollout)
...
class ThinkingModelTipsScheduler(MultiTurnScheduler):
    ...
    def _build_messages(self, original_messages: Messages) -> Messages:
        ...
        if self._is_thinking_template():
            # Create a mock inputs object to use the template's _usf_prepare_inputs method
            class MockInputs:
            ...
            template = self.infer_engine.template
            # _usf_prepare_inputs will remove historical thinking content when in train mode, patch the mode here
            original_mode = template.mode
            template.mode = 'train'
            # Use the template's method to prepare messages
            template._usf_prepare_inputs(mock_inputs)
