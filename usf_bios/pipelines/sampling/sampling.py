...
class USFSampling(USFPipeline):
    ...
    def _get_dataset(self):
        ...
        logger.info_debug(f'Sampling_dataset: {sampling_dataset}')
    ...
    def run(self):
        ...
        if self.args.resume:
            ...
            if os.path.exists(ckpt_state_file):
                with open(ckpt_state_file, 'r') as ckpt_state:
                    ...
                    logger.info_debug(f'Loaded index_resume: {index_resume}')
        else:
        ...
        with open(tmp_file, write_mode) as f:
            for _index in range(self.args.num_sampling_batches):
                if _index <= index_resume:
                    continue
                logger.info_debug(f' Sampling index:{_index}')
        ...
        logger.info_debug(f'Sample file {iter_file} generated.')
