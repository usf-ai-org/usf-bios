...
from usf_bios.megatron.tuners import LoraParallelLinear
from usf_bios.megatron.utils import (adapter_state_dict_context, copy_original_module_weight, patch_merge_fn,
                                  prepare_mcore_model)
from usf_bios.metrics import MeanMetric
from usf_bios.template import Template
from usf_bios.trainers import USFMixin, dynamic_gradient_checkpointing
from usf_bios.utils import is_debug_mode, JsonlWriter, deep_getattr, format_time, get_last_valid_indices, get_logger, ms_logger_context
from .utils import (MegatronPretrainingRandomSampler, get_batch_on_this_cp_rank, get_batch_on_this_tp_rank,
                    get_packed_seq_params, get_usf_datasets_provider)
...
class BaseMegatronTrainer(ABC):
    ...
    def __init__(self, args, template: Template):
        ...
        if is_debug_mode():
            logger.info_debug(f'logging_path: {logging_path}')
        ...
        if (args.check_model and hasattr(args, 'model_info') and hasattr(args.model_info, 'model_dir')
                and not is_local_path and not use_hf):
    ...
    def _get_iters(self, train_dataset, val_dataset):
        ...
        def initialize_megatron(*_args, **kwargs):
            ...
            if args.eval_iters < 0:
                ...
                logger.info_debug(f'Setting args.eval_iters: {args.eval_iters}')
            return res
    ...
    def new_cyclic_iter(self, iterable):
        ...
        while True:
            if not is_finished:
                logger.info_debug(f'The training of Epoch {n_epoch} starts...')
            for x in iterable:
                yield x
            ...
            if is_finished:
                ...
                logger.info_debug(f'Training of {n_epoch} epochs has been completed, the training has finished.')
    ...
    def _get_param_groups(
        ...
        if self.args.vit_lr is not None or self.args.aligner_lr is not None:
            ...
            logger.info_debug(f'vit_lr: {vit_lr}, aligner_lr: {aligner_lr}, llm_lr: {self.args.lr}')
    ...
    def _load_iteration(self):
        ...
        if ckpt_dir is None:
            return 0, 0
        logger.info_debug(f'checkpoint_dir: {ckpt_dir}')
    ...
    def _get_metrics(self, total_loss_dict, mode):
        ...
        return {
            ...
            for k, v in USFMixin.compute_custom_metrics(self.custom_metrics[mode]).items()
        }
    ...
    def _init_multimodal_full(self):
        ...
        if args.train_type == 'full' and args.is_multimodal and visual_cls is not None:
            ...
            if args.freeze_parameters:
                if is_debug_mode():
                    logger.info_debug(f'freeze_parameters: {args.freeze_parameters}')
            if args.trainable_parameters:
                if is_debug_mode():
                    logger.info_debug(f'additional trainable_parameters: {args.trainable_parameters}')

    def train(self, train_dataset, val_dataset, data_collator):
        args = self.args
        datasets_provider = get_usf_datasets_provider(train_dataset, val_dataset)
