...
class RolloutTrainerArgumentsMixin(VllmArguments):
    ...
    Args:
        ...
        vllm_mode (Literal['server', 'colocate']): The vLLM integration mode. 'server' mode uses a vLLM server launched
            by usf rollout for sampling. 'colocate' mode deploys vLLM within the same process. For full-parameter
            training in 'server' mode, the environment variable `USF_UPDATE_WEIGHTS_BUCKET_SIZE` can be set to
...
class GRPOArgumentsMixin(RolloutTrainerArgumentsMixin):
    ...
    Args:
        ...
        scale_rewards (Optional[Literal['group', 'batch', 'none', 'gdpo']]): Specifies the reward scaling strategy.
            ...
            'reinforce_plus_plus'. In usf-bios < 3.10, this was a boolean where `True` corresponded to 'group'
    ...
    # reward function args, see details in usf_bios/plugins/orm.py
