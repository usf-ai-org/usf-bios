...
from usf_bios.arguments import RolloutArguments
from usf_bios.infer_engine import GRPOVllmEngine, InferClient
from usf_bios.infer_engine.protocol import (InitCommunicatorRequest, RequestConfig, RolloutInferRequest,
                                         UpdateWeightsRequest)
from usf_bios.plugins import RolloutScheduler, multi_turns
from usf_bios.rlhf_trainers.utils import (FlattenedTensorBucket, FlattenedTensorMetadata, TensorLoRARequest,
...
try:
    ...
    pass
"""
This module defines the execution logic for `usf rollout`.
...
- For inference or deployment, please use the `usf infer` or `usf deploy` commands.
"""
...
def llm_worker(args: RolloutArguments, data_parallel_rank: int, master_port: int, connection: Connection) -> None:
    ...
    engine = USFRolloutDeploy.get_infer_engine(args, template=args.get_template())
...
class USFRolloutDeploy(USFPipeline):
    ...
    def get_infer_engine(args: RolloutArguments, template=None, **kwargs):
        ...
        if infer_backend != 'vllm':
            infer_backend = 'vllm'
            logger.info_debug('Currently, rollout only supports the vLLM backend. Set vLLM backend')
        ...
        engine_kwargs.update({'worker_extension_cls': 'usf_bios.pipelines.infer.rollout.WeightSyncWorkerExtension'})
