...
This document introduces how to register a multimodal model in usf-bios and successfully perform inference and training. Using Qwen2.5-Omni as an example, we will register a new model_type and template `my_qwen2_5_omni`, supporting training with text, images, videos, and audio. Since Qwen2.5-Omni is already registered in usf-bios, we can use our custom components by explicitly specifying the model_type and template.
...
from usf_bios.model import (Model, ModelGroup, ModelMeta, MultiModelKeys, get_model_processor, register_model,
                         register_model_arch, ModelLoader)
from usf_bios.model.models.qwen import patch_qwen_vl_utils
from usf_bios.model.patcher import patch_get_input_embeddings
from usf_bios.model.utils import use_submodel_func
from usf_bios.utils import get_env_args, Processor
...
Second, we need to register a template to customize how text, images, videos, and audio are preprocessed (`_encode` and `_data_collator` methods). This is a key module for usf-bios's support of multimodal model training. Preprocessing methods should reference transformers inference implementation and align with it.
...
from usf_bios import get_model_processor
from usf_bios.template import StdTemplateInputs, Template, TemplateMeta, get_template, register_template
from usf_bios.template.utils import Context, findall
from usf_bios.template.vision_utils import load_audio
from usf_bios.utils import Processor, get_env_args, get_logger, get_packed_seq_params, is_deepspeed_enabled, to_float_dtype
...
if __name__ == '__main__':
    ...
    input_ids_usf, response_usf = test_my_qwen2_5_omni()
    # Test input_ids and response alignment
    assert input_ids_hf == input_ids_usf
    assert response_hf == response_usf
```
...
usf sft \
    ...
    --dataset 'tatsu-lab/alpaca#2000' \
              'linxy/LaTeX_OCR:human_handwrite#2000' \
              'speech_asr/speech_asr_aishell1_trainsets:validation#2000' \
              'usf_bios/VideoChatGPT:all#2000' \
    --load_from_cache_file true \
    --split_dataset_ratio 0.01 \
    --train_type lora \
    --torch_dtype bfloat16 \
...
Use the following command to push training weights to HuggingFace:
