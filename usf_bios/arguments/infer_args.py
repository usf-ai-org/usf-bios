...
from usf_bios.trainers import VllmArguments
from usf_bios.utils import is_debug_mode, get_logger, init_process_group, is_dist, to_abspath
...
class InferArguments(MergeArguments, LmdeployArguments, SglangArguments, VllmArguments, BaseArguments):
    ...
    Args:
        ...
        reranker_use_activation (bool): Whether to apply a sigmoid activation to the scores during reranker inference.
            Defaults to True.
    """
    # `pt` is used for usf3.x shell script compatibility.
    ...
    def _init_result_path(self, folder_name: str) -> None:
        ...
        if is_debug_mode():
            logger.info_debug(f'args.result_path: {self.result_path}')

    def _init_stream(self):
        self.eval_human = not self._val_dataset_exists
        logger.info_debug(f'Setting args.eval_human: {self.eval_human}')
        if self.stream is None:
            self.stream = self.eval_human
        if self.stream and self.num_beams != 1:
            self.stream = False
            logger.info_debug('Setting args.stream: False')
    ...
    def __post_init__(self) -> None:
        if self.infer_backend == 'pt':
            self.infer_backend = 'transformers'  # compat usf3.x
        ...
        # Default to False for usf infer (non-encode tasks)
