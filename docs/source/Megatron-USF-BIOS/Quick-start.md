...
usf-bios incorporates Megatron's parallelization techniques to accelerate the training of large models, including data parallelism, tensor parallelism, pipeline parallelism, sequence parallelism, context parallelism, and expert parallelism. It supports CPT/SFT/DPO/GRPO for models such as Qwen3, [Qwen3-MoE](https://github.com/us-inc/usf-bios/blob/main/examples/megatron/mcore_bridge/full/moe.sh), Qwen2.5, Llama3, Deepseek-R1 and GLM4.5 series. For a complete list of supported models, please refer to the [Supported Models and Datasets documentation](../Instruction/Supported-models-and-datasets.md). We recommend using Megatron-USF BIOS for MoE training; it can typically achieve a 10x speedup in training.

| Method                 | Full-Parameter | LoRA | MoE  | Multimodal | FP8  |
| ---------------------- | -------------- | ---- | ---- | ---------- | ---- |
| Pre-training           | ✅              | ✅    | ✅    | ✅          | ✅    |
| [Supervised Fine-Tuning](https://github.com/us-inc/usf-bios/tree/main/examples/megatron) | ✅              | ✅    | ✅    | ✅          | ✅    |
| [GRPO](https://github.com/us-inc/usf-bios/tree/main/examples/megatron/grpo)                   | ✅              | ✅    | ✅    | ✅          | ✅    |
| [GKD](https://github.com/us-inc/usf-bios/tree/main/examples/megatron/rlhf/gkd)                    | ✅              | ✅    | ✅    | ✅          | ✅    |
| [DPO](https://github.com/us-inc/usf-bios/tree/main/examples/megatron/rlhf/dpo)                    | ✅              | ✅    | ✅    | ✅          | ✅    |
| [KTO](https://github.com/us-inc/usf-bios/tree/main/examples/megatron/rlhf/kto)                    | ✅              | ✅    | ✅    | ✅          | ✅    |
| [RM](https://github.com/us-inc/usf-bios/tree/main/examples/megatron/rlhf/rm)                     | ✅              | ✅    | ✅    | ✅          | ✅    |
| [Sequence Classification](https://github.com/us-inc/usf-bios/tree/main/examples/megatron/seq_cls)    | ✅              | ✅    | ✅    | ✅          | ✅    |

## Environment Setup

To use Megatron-USF BIOS, in addition to installing the `usf` dependencies, you also need to install the following:

```shell
pip install pybind11

# transformer_engine
# If an installation error occurs, you can refer to this issue for resolution: https://github.com/us-inc/usf-bios/issues/3793
pip install --no-build-isolation transformer_engine[pytorch]

# apex
# Note: Megatron-USF BIOS can run in environments without apex by setting `--no_gradient_accumulation_fusion true`.
...
# If you are using multi-node training, please additionally set the `HF_HOME` environment variable to a shared storage path.
...
Alternatively, you can also use the image: (See historical images [here](../GetStarted/USF BIOS-installation.md#mirror))
```
...
```
<<< who are you?
I am a language model developed by usf, you can call me usf-robot. How can I assist you?
```

### Mcore-Bridge [Recommended]

In "usf-bios>=3.10", Mcore-Bridge is supported, eliminating the cumbersome process of model conversion. For details, refer to [Mcore-Bridge Documentation](./Mcore-Bridge.md).
...
- Megatron-USF BIOS uses the same dataset and template processing modules as usf-bios, thus supporting techniques such as packing, loss scale, and agent training. For custom dataset formats, please refer to the [Custom Dataset Documentation](../Customization/Custom-dataset.md).
- **More Examples**: Including packing, multi-node training, 32K context length, DPO, MoE models, and pre-training, can be found [here](https://github.com/us-inc/usf-bios/tree/main/examples/megatron).
...
- Parallelism choices:
  - Megatron-USF BIOS uses ZeRO-1 (use_distributed_optimizer enabled by default) combined with various parallelism techniques.
...
- Choosing parallelism for weight conversion: Megatron-USF BIOS uses the torch_dist storage format on the MCore side; you can adjust parallelism at training time and do not need to specify it during weight conversion.
- Regarding log printing: Megatron-USF BIOS logs are printed on the last rank, because in PP parallelism, only the last pp_rank has complete information.

## Benchmark
The training speed comparison for full-parameter dense models with 8K context length, using `megatron sft` and `usf sft`, under a single-node, eight-GPU A800 environment is as follows:
...
The training speed comparison for full-parameter MoE models with 8K context length, using `megatron sft` and `usf sft`, under a two-node, 16-GPU A800 environment is as follows:
...
## Megatron-USF BIOS Wechat Group
