...
def test_dataset(infer_backend):
    ...
    args = InferArguments(
        ...
        val_dataset=['tatsu-lab/alpaca#10'],
        stream=True)
    infer_main(args)
...
def test_mllm_dataset(infer_backend):
    ...
    args = InferArguments(
        ...
        infer_backend=infer_backend,
        val_dataset=['detection-datasets/coco:validation#1000'],
        stream=True)
    infer_main(args)
...
def test_dataset_ddp():
    ...
    from usf_bios import infer_main, InferArguments
    args = InferArguments(
        model='Qwen/Qwen2-7B-Instruct', max_batch_size=64, val_dataset=['tatsu-lab/alpaca#1000'])
    infer_main(args)
