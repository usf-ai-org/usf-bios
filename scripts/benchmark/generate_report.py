...
def parse_output(file):
    ...
    if cmd == 'export':
        ...
        if 'model_info' in content:
            # model_info like: USFModel: 6758.4041M Params (19.9885M Trainable [0.2958%]), 16.7793M Buffers.
            str_dict = split_str_parts_by(content['model_info'], [
                'USFModel:', 'CausalLM:', 'Seq2SeqLM:', 'LMHeadModel:', 'M Params (', 'M Trainable [', ']), ',
                'M Buffers.'
            ])
            str_dict = {c['key']: c['content'] for c in str_dict}
            if 'USFModel:' in str_dict:
                num_total_parameters = float(str_dict['USFModel:'])
