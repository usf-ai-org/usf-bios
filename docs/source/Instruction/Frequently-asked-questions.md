...
> [!WARNING]
> This document is pending update to usf-bios 4.0

Here are some common questions encountered during the use of USF BIOS.

## Training

### Q1: What models and datasets are supported for fine-tuning in USF BIOS?
Please refer to the documentation on [Supported Models and Datasets](https://usf.readthedocs.io/en/latest/Instruction/Supported-models-and-datasets.html).

### Q2: What data formats are supported when training with custom datasets?
For custom dataset formats, see the documentation on [Custom Dataset](https://usf.readthedocs.io/en/latest/Customization/Custom-dataset.html).

### Q3: What is the format for dataset_info.json for custom datasets, and how can I use it?
The dataset_info.json format can be found in the documentation on [Custom Dataset](https://usf.readthedocs.io/en/latest/Customization/Custom-dataset.html). Use the command line with `--custom_dataset_info xxx.json`, `--dataset <dataset_id_or_path>`.

### Q4: How can I train with a custom dataset using the interface?
Using a custom dataset through the interface is the same as using the command line. Refer to the documentation on [Custom Dataset](https://usf.readthedocs.io/en/latest/Customization/Custom-dataset.html).

### Q5: Can I write a line in the jsonl file like this? {"index": "00000", "query": "11111", "response": "22222", 'source':'qqq'}
Yes, you can have extra fields. By default, these fields will not be used. For details, see the parameter [remove_unused_columns](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html#data-arguments).

### Q6: Where can I find the command line parameters?
Please refer to the documentation on [Command Line Parameters](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html).

### Q7: What parameters need to be configured for training in an offline environment?
Use `--model local_path`, `--check_model false`. For more details, see the [Command Line Parameters](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html).

### Q8: Where can I check model_type?
Check the [Supported Models and Datasets](https://usf.readthedocs.io/en/latest/Instruction/Supported-models-and-datasets.html).

### Q9: Can I directly convert the model to gguf format after training?
Currently, only export to ModelFile is supported. See the [Command Line Parameters](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html).

### Q10: Does USF BIOS support pre-training? I only see SFT.
Yes, it supports it. Use the command line `usf pt`, [pretrain example](https://github.com/us-inc/usf-bios/tree/main/examples/train/pretrain). The dataset format is detailed in [Custom Dataset](https://usf.readthedocs.io/en/latest/Customization/Custom-dataset.html).
...
You do not need to merge. Use `--resume_from_checkpoint output/xxx/vx-xxx/checkpoint-xxx`. See the [Command Line Parameters](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html).
...
You can set the environment variable `HF_HOME=your_path` to store the original model in the specified path. For SDK downloads, use `cache_dir="local_path"`. You can also use the `huggingface download` command-line tool or `git` to download it. For details, refer to the [Download Model](https://huggingface.co/docs/Models/Download-Model). During training, set `--model` to the local path. For offline training, configure `--check_model false`. See the [Command Line Parameters](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html).

### Q14: Is there a complete tutorial and command line for fine-tuning Qwen-2-VL?
Reference the [example](https://github.com/us-inc/usf-bios/tree/main/examples/train/multimodal) for multimodal model training.
...
### Q17: Official Magic Mirror image and USF BIOS environment.
You can start the container using `docker run`, for example: `docker run --gpus all -p 8000:8000 -it -d --name ms huggingface-registry.cn-beijing.cr.aliyuncs.com/huggingface-repo/huggingface:ubuntu22.04-cuda12.4.0-py311-torch2.6.0-1.26.0-LLM /bin/bash`. After starting the container, pull the latest code to install USF BIOS. Additionally, for large model training scenarios, the `usf-bios` image is provided, which includes additional dependencies for `Megatron-USF BIOS`, such as: `huggingface-registry.cn-beijing.cr.aliyuncs.com/huggingface-repo/huggingface:ubuntu22.04-cuda12.4.0-py311-torch2.6.0-vllm0.8.5.post1-huggingface1.26.0-usf3.4.1.post1`. For more details, refer to the [USF BIOS installation documentation](https://usf.readthedocs.io/en/latest/GetStarted/USF BIOS-installation.html).

### Q18: Command line for multi-machine multi-card training.
For details, see the [Multi-node Example](https://github.com/us-inc/usf-bios/tree/main/examples/train/multi-node).

### Q19: How to choose a template?
See [issue](https://github.com/us-inc/usf-bios/issues/1813).

### Q20: How to use torchrun and usf sft for multi-card training?
`usf sft` uses `torchrun`.

### Q21: I have a question about my SFT dataset being too large; tokenizing takes a long time. Is there a solution?
Use `lazy_tokenize`or stream reading (`streaming`). See [Command Line Parameters documentation](https://usf.readthedocs.io/zh-cn/latest/Instruction/Command-line-parameters.html).
...
Command-line parameter `dataset_shuffle`. For more details, see the [command-line parameters documentation](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html).
...
Yes, you can. Please refer to [Best Practices for MLLM Registration](https://usf.readthedocs.io/en/latest/BestPractices/MLLM-Registration.html).

### Q26: Can I use DPO to train Qwen2-VL in a Python script?
Yes, import `rlhf_main` and `RLHFArguments` from `usf.pipelines`.
...
### Q29: Does USF BIOS support distillation?
Refer to this [example](https://github.com/us-inc/usf-bios/blob/main/examples/sampler/distill/distill.sh).

### Q30: How many checkpoints are saved by default after training?
By default, all checkpoints are saved. For details, see the [command-line parameter save_total_limit](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html). This parameter is not supported in Megatron-USF BIOS; please set save_interval to save checkpoints instead. For details, refer to the [Megatron-USF BIOS command-line parameters](https://usf.readthedocs.io/en/latest/Megatron-USF BIOS/Command-line-parameters.html).

### Q31: In grounding tasks, does the universal data format support multiple instances for one category?
Currently, it supports one object corresponding to multiple bounding boxes. Refer to the documentation on [Custom Dataset](https://usf.readthedocs.io/en/latest/Customization/Custom-dataset.html#grounding).

### Q32: Why am I getting the error that numpy.object cannot be found?
Try using `numpy==1.26.3`.

### Q33: Does the USF BIOS framework support sequence parallelism now?
Yes, it supports it. Refer to the [example](https://github.com/us-inc/usf-bios/tree/main/examples/train/sequence_parallel) here.
...
Refer to the QLoRA [example](https://github.com/us-inc/usf-bios/tree/main/examples/train/qlora).

### Q37: How do I expand my vocabulary within the USF BIOS framework?
Please refer to the command-line-parameters documentation for [new_special_tokens](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html#model-arguments) for more information.
...
### Q41: Can I save the inference results of the validation set during training in USF BIOS?
After training, run `usf infer` to save the results.
...
### Q43: Training speed slows down when using multi-machine training; using USF BIOS framework for LLM training with deepspeed zero3 causes significant performance drop.
See the [issue](https://github.com/us-inc/usf-bios/issues/1825).

### Q44: Does USF BIOS now support multi-stage pre-training for qwen2-vl? It looks like the official best practices only show SFT training with vit+llm together, not sure if separate fine-tuning is supported.
You can control this using the parameters `--freeze_vit`, `--freeze_aligner`, and `--freeze_llm`. For more details, see the [Command Line Parameters Documentation](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html#tuner-arguments).
...
Refer to the [Pre-training and Fine-tuning](https://usf.readthedocs.io/en/latest/Instruction/Pre-training-and-Fine-tuning.html). If you notice repetitions during training, try training for more epochs, cleaning the data, and conducting full parameter training, using RLHF to mitigate this issue.

### Q48: Does USF BIOS currently support prompt tuning or prefix tuning?
...
### Q51: Does USF BIOS have a dataset quality inspection tool?
[data-juicer](https://github.com/huggingface/data-juicer).
...
`dataset_path` supports folders, typically for datasets downloaded via `git clone`. See [Custom Dataset Documentation](https://usf.readthedocs.io/en/latest/Customization/Custom-dataset.html#dataset-info-json).
...
See the streaming parameter description, [Command Line Parameters Documentation](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html#data-arguments).
...
This parameter no longer exists in usf3.0, use the `strict` parameter instead.
...
```
Add the parameter `--gradient_checkpointing_kwargs '{"use_reentrant": false}'`. Alternatively, upgrade usf-bios, which sets this parameter by default.
...
### Q61: Does LlamaPro in usf support multimodal adaptation?
Yes, it's supported.
...
Environment variable parameters correspond to model parameters. `MAX_PIXELS` only supports qwen2vl, internvl has its own environment variables. See [Specific Model Parameters](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html#specific-model-argumen).

### Q63: Is there documentation for fine-tuning qwen base model to chat model? Any special configurations needed?
Use `usf sft`, no special configuration needed. See [example](https://github.com/us-inc/usf-bios/tree/main/examples/train/base_to_chat).

### Q64: Where can I find sequence parallel examples?
See this example: [sequence_parallel](https://github.com/us-inc/usf-bios/tree/main/examples/train/sequence_parallel).

### Q65: Can usf support training custom model structures?
...
### Q67: How to add custom special tokens in usf?
Please refer to the command-line-parameters documentation for [new_special_tokens](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html#model-arguments) for more information.
...
### Q72: Does usf3.0 not support get_default_template_type?
...
Yes, there's an [example](https://github.com/us-inc/usf-bios/blob/main/examples/train/multimodal/grounding.sh) under examples.

### Q75: Does usf-bios support contrastive learning for training llm_emb?
Yes, here's an [example](https://github.com/us-inc/usf-bios/blob/main/examples/train/embedding).

### Q76: Is there a big difference in performance between manually coding fine-tuning and GRPO using peft and trl libraries compared to USF BIOS official training with the same parameters?
The difference is minimal, with USF BIOS additionally supporting multimodality.

### Q77: Does USF BIOS currently not support audio modal input training for minicpmo2_6? It shows error: assert media_type in {'image', 'video'}
Audio is not currently supported.

### Q78: Can USF BIOS fine-tune deepseek R1 671B?
Yes, the template is integrated, but the process is complicated as it requires converting fp8 to bf16 first.

### Q79: Isn't the latest USF BIOS framework supposed to specify the model location using this command? This is the location of the model I've already downloaded, but I don't know why it still tries to download and fails with a git clone error
```shell
--model /mnt/workspace/.cache/huggingface/hub/deepseek-ai/deepseek-vl2/ \
```
Some models require cloning the repo and then specifying through `local_repo_path`.

### Q80: Does USF BIOS now support multimodal GRPO?
Yes, it does.

### Q81: Can the GRPO reward function be customized?
Yes, refer to [examples/train/grpo/plugin](https://github.com/us-inc/usf-bios/tree/main/examples/train/grpo/plugin).
...
### Q83: I have a question. I trained a reward model using USF BIOS (baseline is qwen2.5-7b), but when loading it in PPO or GRPO, it shows an error. The reward model was trained using LoRA.
```shell
--rlhf_type ppo \
--model Qwen/Qwen2.5-14B-Instruct \
--reward_model /mnt/workspace/output/rm/model --train_type lora \
--dataset 'tatsu-lab/alpaca#20000' --torch_dtype float32 --num_train_epochs 1 \
...
### Q85: Generate train split is too slow (about 30+ datasets with around a million total data points). Previously USF BIOS 2.x wasn't this slow. Lazy tokenize is already enabled.
...
Refer to this [example](https://github.com/us-inc/usf-bios/tree/main/examples/train/multimodal/lora_llm_full_vit).

### Q87: How to use custom loss functions in USF BIOS?
...
### Q90: Getting errors when fine-tuning Moonlight-16B-A3B-Instruct model. Seems usf-bios doesn't support fine-tuning this model?
...
Set `--max_steps` or `--max_epochs`. For more details, see the streaming parameter description in the [Command Line Parameters Documentation](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html#data-arguments).
...
### Q103: Does USF BIOS support multi-label classification now?
...
### Q106: Does usf currently support sequence parallelism for those reinforcement learning training methods?
...
### Q111: If you use the default num_iterations=1, does clip become ineffective? The clip higher in dapo is also useless. I see that veRL has a micro batch setting to update the policy model in small batches for the clip term to take effect. In usf-bios, it seems mini batch only does gradient accumulation according to the source code?
...
### Q116: Can usf cache a mapped version of data for troubleshooting training data issues?
...
```json
{"messages": [{"role": "assistant", "content": "<video>"}], "videos": [["1.jpg","2.jpg"]]}
```
...
Check this [issue](https://github.com/us-inc/usf-bios/issues/4030).

### Q125: How do I configure resuming training from a checkpoint in megatron-usf?
Configure `--load` to load the checkpoint. Additionally, configure these parameters as needed: `--finetune`, `--no_load_optim`, and `--no_load_rng`. For resuming LoRA training, configure --adapter_load; other settings are the same as for full-parameter training. For details, refer to the [megatron-usf command-line-parameters documentation](https://usf.readthedocs.io/en/latest/Megatron-USF BIOS/Command-line-parameters.html).
...
### Q127: When training qwenvl2.5, how can I ensure the bounding box coordinates are correct after setting max_pixels? How does USF BIOS handle this?
USF BIOS saves the states before and after preprocessing and adjusts the bounding box accordingly. However, this adjustment is not performed during inference, so you need to manually preprocess the images in advance.
...
Yes, they conflict. Sequence parallelism has its own customized loss implementation. You can modify it yourself [here](https://github.com/us-inc/usf-bios/blob/main/usf/trainers/sequence_parallel/ulysses.py).

### Q133: In USF BIOS's PT (pre-training) mode, when an <image> token is passed, is it not masked, meaning it contributes to the loss calculation?
...
You can add new losses [here](https://github.com/us-inc/usf-bios/blob/main/usf/loss/mapping.py).
...
### Q146: In the command-line arguments for USF BIOS SFT, are LoRA training and the --trainable_parameters argument compatible? For instance, I want to use LoRA to train the language model while also adding a score head to the final layer and training it.
...
### Q148: Can USF BIOS support setting a minimum learning rate? I feel like it decreases too much in the end.
...
See [Multi-task Training](https://usf.readthedocs.io/en/latest/Instruction/GRPO/DeveloperGuide/multi_task.html).
...
### Q153: Does USF BIOS support multi-node distributed training?
Refer to the [example](https://github.com/us-inc/usf-bios/tree/main/examples/train/multi-node) here.

### Q154: When fine-tuning a VLM with several tasks together, the video sampling rules are different for each task. Does MS-USF BIOS support this? And where can I configure it?
Check the interleave_prob parameter in the [Command-line Parameters Documentation](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html).
...
### Q158: In USF BIOS, where is the location in the code that reads an image and converts it into a tensor?
...
### Q160: I have a question about fine-tuning the qwen3-30b-a3b MoE model with LoRA in MS-USF BIOS. The aux_loss barely changes, even when I set aux_loss_coef to 1.
...
It depends on whether the gate is implemented as nn.Linear. If it's an nn.Parameter, it won't be trained. For details, see the command-line parameter [target_parameters](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html#tuner-arguments).
...
### Q1: Is there documentation for USF BIOS inference?
USF BIOS supports inference via Python scripts, command line, and UI interface. See the [Inference and Deployment](https://usf.readthedocs.io/en/latest/Instruction/Inference-and-deployment.html).

### Q2: How to use the trained model for inference with a dataset?
Use the parameters `--load_data_args true` or `--val_dataset <your-val-dataset>`. Refer to the [Command Line Parameters](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html).

### Q3: Can I specify a locally saved model during USF BIOS inference?
Set `--model` to the local path. See [Command Line Parameters](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html).
...
Set the command line parameter `--max_pixels xxx`, environment variable `MAX_PIXELS=xxx`, or specific model parameter `--model_kwargs '{"max_pixels": xxx}'`. Note that the environment variable only takes effect for the corresponding models in the documentation. For more details, please refer to the documentation [Specific Model Parameters](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html#specific-model-arguments).

### Q7: On a V100 GPU, in a Python virtual environment, following the environment setup instructions from https://usf2x.readthedocs.io/zh-cn/latest/Multi-Modal/qwen2-vl%E6%9C%80%E4%BD%B3%E5%AE%9E%E8%B7%B5.html, when testing the inference command: `CUDA_VISIBLE_DEVICES=0,1,2,3 usf infer --model_type qwen2-vl-7b-instruct`, an error occurs: `RuntimeError: probability tensor contains either 'inf', 'nan' or element < 0`.
Try inference on A10 or 3090 machines.

### Q8: After running the prediction command, where are the results saved? CUDA_VISIBLE_DEVICES=0 usf infer --ckpt_dir output/glm4v-9b-chat/vx-xxx/checkpoint-xxx-merged --load_data_args true
Results will be printed in the log.

### Q9: For the latest version of usf, can the infer command output probability values through the logprobs parameter?
Yes, logprobs can be output. For command line inference, set `--logprobs true`. For Python script inference, set `request_config = RequestConfig(..., logprobs=True, top_logprobs=2)`. Please refer to [test_logprobs.py](https://github.com/us-inc/usf-bios/blob/main/tests/infer/test_logprobs.py).
...
Models do not have to be merged. See the documentation on [Command Line Parameters](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html).
...
Yes, see the [issue](https://github.com/us-inc/usf-bios/issues/1653).

### Q15: Where to set `do_sample` for multi-modal inference using inference client?
Set `temperature=0`.

### Q16: Does usf-bios support batch processing for large models?
Supported. See the [demo](https://github.com/us-inc/usf-bios/blob/main/examples/infer/demo.py).

### Q17: When quantizing models with usf-bios, there is an insufficient memory display. Can we reduce resource usage during quantization, even if it's slower?
Try setting `--device_map cpu`.

### Q18: Does USF BIOS support quantization for multi-modal models?
...
### Q20: How can I specify where to save evaluation results during usf infer? I can't find where the results are saved.
Set `--result_path your_path`. See [InferArguments](https://github.com/us-inc/usf-bios/blob/main/usf/arguments/infer_args.py).

### Q21: I get an error while using AWQ quantized yi-vl-6b:
```text
TypeError: usf.llm.utils.model.get_model_tokenizer_with_flash_attn() got multiple values for keyword argument 'automodel_class'.
```
Please use GPTQ quantization.

### Q22: I would like to ask about using usf export to perform GPTQ INT4 quantization on the qwen2.5 72B model with a max model length of 32768, which is the default value. The calibration dataset provided has 128 samples, but an error occurred during quantization. The error log is: "factorization could not be completed because the input is not positive-definite (the leading minor of order 18145 is not positive-definite)." What is the cause?
...
Yes, it can be done using `usf infer --val_dataset xxx --max_batch_size 16 ...`.

### Q24: What's the default temperature value when using usf app for inference?
...
### Q26: When using usf export with a custom template_type, does it permanently change the template_type? If we use usf export --template_type custom, does it change the model's template?
No, it won't be modified. Templates in usf are defined internally, not saved in jinja format.
...
### Q29: Does usf.inference now support messages format input? It seems to only support query format currently. The answer contains part of the prompt, how should I modify the inference to complete the answer?
```text
{"messages": [{"role": "system", "content": "<system>"}, {"role": "user", "content": "<query1>"}, {"role": "assistant", "content": "answer1, "}]}
```
This is supported in usf3, refer to [examples/infer/demo_agent](https://github.com/us-inc/usf-bios/blob/main/examples/infer/demo_agent.py).

### Q30: How can I make usf infer write results to result_path in real-time instead of writing everything at once at the end?
```shell
usf infer \
--ckpt_dir model_dir \
--streaming true \
...
### Q31: When I trained and did inference in USF BIOS it worked, but after merge_lora when using Ollama's API the effect disappeared.
Try loading with transformers, USF BIOS's template is aligned with transformers.
...
```text
File "/mnt/workspace/usf/usf/1lm/dataset/preprocessor/core. py", line 69, in _check_messages raise
ValueError(f'assistant_message; {assistant_message}')
ValueError: assistant_message: {'role' :'assistant', 'content': ''}
```
```shell
CUDA_VISIBLE_DEVICES=0 NPROC_PER_NODE=1 MAX_PIXELS=1003520 usf sft --model Qwen/Qwen2.5-VL-7B-Instruct --train_type lora --dataset /mnt/workspace/data.json --deepspeed zero2 --max_length 16384
```
...
### Q35: When using usf sample, it seems that batch processing is not supported? It appears to sample examples one by one in a loop, which is somewhat slow.
There is a [script](https://github.com/us-inc/usf-bios/blob/main/examples/train/rft/rft.py) that can use multiprocessing to split and sample the dataset.

### Q36: Does USF BIOS support inference for embedding models?
Please refer to the [example](https://github.com/us-inc/usf-bios/blob/main/examples/infer/demo_embedding.py).

### Q37: Does the usf framework support model or tensor parallelism for inference? There is no OOM during training, but OOM occurs during inference.
```shell
...
Downloading Model from https://huggingface.co to directory:/mnt/workspace/.cache/huggingface/hub/models/AIDC-AI/Ovis2-2B
```
...
### Q41: When using USF BIOS for quantization, which components—activations or weights—are quantized by the GPTQ, AWQ, and FP8 methods respectively?
Only the weights.

### Q42: When using usf-bios for inference, I'm seeing a large discrepancy in the results between the Transformers engine and the vLLM engine. What could be the reason?
...
### Q43: When using USF BIOS for Qwen2-Audio inference, the output is garbled/chaotic. What could be the potential cause?
...
### Q46: A model trained with USF BIOS cannot be loaded via US Inc. How can this be resolved?
For details, see [issue#5440](https://github.com/us-inc/usf-bios/issues/5440). LoRA adapters trained with transformers==4.55.2 can no longer be loaded by versions older than 4.52.
...
### Q49: Does the usf infer command support distributed inference across multiple nodes?
...
Please refer to the [inference parameter metric](https://usf.readthedocs.io/en/latest/Instruction/Command-line-parameters.html#inference-arguments).
...
Use `usf deploy --adapters xxx`. Refer to the documentation on [Inference and Deployment](https://usf.readthedocs.io/en/latest/Instruction/Inference-and-deployment.html).

### Q2: How to use vllm for multi-card deployment?
For details, see the [example](https://github.com/us-inc/usf-bios/tree/main/examples/deploy).

### Q3: How can clients pass images during vllm deployment?
See [client examples](https://github.com/us-inc/usf-bios/tree/main/examples/deploy/client/mllm) for details..
...
### Q5: After launching the server with usf deploy using two cards, when I exit with Ctrl+C, there is always a Python process that continues to occupy the memory of one card. Is this a normal phenomenon?
...
### Q8: After starting the usf inference service, how can I set configurations like temperature interactively?
...
base64, see [mllm client example](https://github.com/us-inc/usf-bios/tree/main/examples/deploy/client/mllm) for details.
...
### Q11: When using USF BIOS deploy for inference, I want to output token probabilities. I added logprobs True, but it outputs null. What's the reason?
```shell
RAY_memory_monitor_refresh_ms=0 CUDA_VISIBLE_DEVICES=1 nohup usf deploy --ckpt_dir /mnt/workspace/checkpoint_600 --infer_backend vllm --logprobs True --load_data_args false --host 0.0.0.0 --port 8000 &
```
Parameters need to be passed from the client side, `request_config = RequestConfig(..., logprobs=True, top_logprobs=2)`.

### Q12: Can we set request timeout time for USF BIOS3.0 deployment inference? What happens if the image URL is invalid?
You can set the `USF BIOS_TIMEOUT` environment variable. Alternatively, you can pass parameters in `InferClient`.

### Q13: Why can't I get streaming generation with USF BIOS deployed models? I've set stream to True on both server and client side, but it's still not streaming
It's controlled by the client side. Please check [examples/deploy/client](https://github.com/us-inc/usf-bios/tree/main/examples/deploy/client).

### Q14: After deploying a multimodal model with USF BIOS, is there an example of passing PIL.Image from the client?
Check this [client example](https://github.com/us-inc/usf-bios/blob/main/examples/deploy/client/mllm/openai_client.py).
...
### Q16: When deploying using usf deploy with the parameter --infer_backend vllm, the performance was nearly 10 points worse compared to deploying directly with vllm: vllm serve. Does anyone know the reason for this?
...
### Q18: When I use usf-bios for vllm deployment inference, it is much slower compared to native vllm. Is this a problem with the usf framework?
...
### Q19: If a USF BIOS deployment is accelerated with vLLM, can I specify the proportion of GPU memory to be used on different cards individually?
...
### Q22: After deploying a model using the USF BIOS Transformers engine, inference is not parallelized, and data is not distributed to other GPUs. Everything is running on the first GPU.
Try using usf infer. The deploy command does not support DDP.

### Q23: For a model deployed with usf deploy, how can I disable the "thinking" status on the client side? Adding it to the extra_body of the request doesn't work.
Currently, the "thinking" status can only be disabled when launching usf deploy.

## Evaluation

### Q1: What evaluation datasets are supported by USF BIOS?
...
See the document [Evaluation](https://usf.readthedocs.io/en/latest/Instruction/Evaluation.html) for details.

### Q2: How to use a custom evaluation dataset?
Custom evaluation datasets, both plain text and multimodal, must match the data format (pattern) of an official dataset. See the document [Evaluation](https://usf.readthedocs.io/en/latest/Instruction/Evaluation.html) for details.
...
`pip3 install evalscope==0.5.3.post1`.

### Q4: After manually downloading the official evaluation dataset, can USF BIOS eval be configured for local path evaluation?
First, download the evaluation dataset [eval.zip](https://huggingface.co/datasets/usf/evalscope_resource/files), extract it, and place its contents in the `~/.cache/huggingface/media_resources/evalscope/data` folder. Then execute the `usf eval` command to use the local data.

### Q5: Is there a bug with custom evaluation? I modified the standard example to English, but it doesn't work?
```shell
usf eval --model_type 'qwen2_5-1_5b-instruct' --eval_dataset no --custom_eval_config '/mnt/workspace/test_data/config_eval.json'
```
...
Set the `USF BIOS_TIMEOUT` environment variable to -1.
...
### Q9: Does usf-bios have methods to test QPS, latency, and tokens/s?
...
### Q14: In usf eval, the model stops generating after 1024 tokens. How can I modify this? Setting --max_new_tokens 5000 doesn't seem to work.
This parameter hasn't been exposed in usf yet. You can use evalscope to run it, and configure max_tokens in the model according to the [documentation](https://evalscope.readthedocs.io/en/latest/user_guides/backend/vlmevalkit_backend.html#configure-model-evaluation-parameters).
...
```shell
 ...
 ```
If you want to use datasets locally, it's recommended to clone the repository from huggingface and then specify the path.
...
### Q19: Does usf eval with --eval_backend OpenCompass not support custom datasets?
```text
...
```
This occurs during the download of nltk dependencies. Run `python -c "import nltk; nltk.download('punkt_tab')"` or manually download the punkt_tab data from NLTK and unzip it under `~/nltk_data/tokenizers`.
...
Please refer to this [issue](https://github.com/us-inc/evalscope/issues/610).

### Q29: When using usf eval for benchmark evaluation, can I specify an llm as a judge and how should I pass in the parameters?
Yes, you can use usf to pass `judge-model-args` parameters from `extra_eval_args`, which include `api_key, api_url, and model_id`, as a JSON string.
...
```shell
NPROC_PER_NODE=8
ASCEND_RT_VISIBLE_DEVICES=0,1,2,3,4,5,6,7\ MAX_PIXELS=802816\ usf eval\
...
```
usf eval does not support being launched in DDP mode.
...
### Q37: Where can I see what extra fields, besides the question itself, are included in the query sent during a USF BIOS evaluation?
