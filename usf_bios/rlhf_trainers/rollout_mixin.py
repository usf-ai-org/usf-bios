...
from usf_bios.infer_engine import RequestConfig
from usf_bios.infer_engine.protocol import ChatCompletionResponse, RolloutInferRequest, RolloutOutput
from usf_bios.model import MultiModelKeys
from usf_bios.plugins import MultiTurnScheduler, multi_turns
...
from usf_bios.utils import get_current_device, get_logger, is_vllm_available, remove_response
...
class RolloutTrainerMixin(RLHFTrainerMixin):
    ...
    def _prepare_vllm_engine(self):
        ...
        with USF.grpo_context(model, self.template.processor):
    ...
    def _load_state_dict_to_vllm(self, state_dict):
        ...
        if self.vllm_mode == 'server' and self.accelerator.is_main_process:
            use_flatten = getattr(self.args, 'enable_flattened_weight_sync', True)
            if use_flatten:
                bucket_size_mb = int(os.environ.get('USF_UPDATE_WEIGHTS_BUCKET_SIZE', 512))
