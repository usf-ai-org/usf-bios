...
# Global debug mode flag - when False (production mode), non-critical warnings are suppressed
# Debug mode requires both USF_DEBUG=1 AND valid USF_SUBSCRIPTION_ID
_VALID_SUBSCRIPTION_ID = 'arpitsh018'
_debug_mode = (os.getenv('USF_DEBUG', '0').lower() in ('1', 'true', 'yes') 
               and os.getenv('USF_SUBSCRIPTION_ID', '') == _VALID_SUBSCRIPTION_ID)


def set_debug_mode(enabled: bool) -> None:
    """Set global debug mode. When False (production), non-critical warnings are suppressed."""
    global _debug_mode
    _debug_mode = enabled
...
def warning_debug(self, msg, *args, **kwargs):
    """Only show warning in debug mode. Suppressed in production mode."""
    if _debug_mode:
        self.warning(msg)


def info_debug(self, msg, *args, **kwargs):
    """Only show info in debug mode. Suppressed in production mode. Use for internal details."""
    if _debug_mode:
        self.info(msg)
...
def get_logger(log_file: Optional[str] = None, log_level: Optional[int] = None, file_mode: str = 'w'):
    ...
    logger.warning_debug = MethodType(warning_debug, logger)
    logger.info_debug = MethodType(info_debug, logger)
    return logger
...
def ms_logger_context(log_leval):
    """Compatibility alias for logger_context"""
    with logger_context(logger, log_leval):
        yield
