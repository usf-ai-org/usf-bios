...
Below are the inference engines supported by USF BIOS along with their corresponding capabilities. The three inference acceleration engines provide inference acceleration for USF BIOS's inference, deployment, and evaluation modules:
...
| ------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | --------------- | ------------------------------------------------------------ | ----- | ------------------------------------------------------------ | ------------------- |
| transformers                               | [✅](https://github.com/us-inc/usf-bios/blob/main/examples/deploy/client/llm/chat/openai_client.py) | [✅](https://github.com/us-inc/usf-bios/blob/main/examples/app/mllm.sh) | ✅               | [✅](https://github.com/us-inc/usf-bios/blob/main/examples/infer/demo_lora.py) | ✅     | [✅](https://github.com/us-inc/usf-bios/blob/main/examples/infer/transformers/batch_ddp.sh) | DDP/device_map      |
| [vllm](https://github.com/vllm-project/vllm)     | ✅                                                            | [✅](https://github.com/us-inc/usf-bios/blob/main/examples/infer/vllm/mllm_tp.sh) | ✅               | [✅](https://github.com/us-inc/usf-bios/blob/main/examples/deploy/lora/server.sh) | ❌     | ✅                                                            | TP/PP/DP               |
| [sglang](https://github.com/sgl-project/sglang)    | ✅          | ❌ |      ✅        | ❌      | ❌     | ✅ | TP/PP/DP/EP |
| [lmdeploy](https://github.com/InternLM/lmdeploy) | ✅                                                            | [✅](https://github.com/us-inc/usf-bios/blob/main/examples/infer/lmdeploy/mllm_tp.sh) | ✅               | ❌                                                            | ❌     | ✅                                                            | TP/DP                  |

## Inference

usf-bios uses a layered design philosophy, allowing users to perform inference through the command-line interface, web UI, or directly using Python.
...
Input an image path or URL <<< https://placehold.co/400x300/png?text=cat
Input an image path or URL <<< https://placehold.co/400x300/png?text=animal
...
Input a video path or URL <<< https://placehold.co/400x300/mp4?text=video
...
CUDA_VISIBLE_DEVICES=0 usf infer \
    ...
    --stream true \
    --infer_backend transformers \
    --val_dataset tatsu-lab/alpaca \
    --max_new_tokens 2048
```

The above example provides streaming inference for both full parameters and LoRA, and below are more inference techniques available in USF BIOS:

- Interface Inference: You can change `usf infer` to `usf app`.
- Batch Inference: For large models and multimodal models, you can specify `--max_batch_size` for batch inference by using `infer_backend=transformers`. For specific details, refer to [here](https://github.com/us-inc/usf-bios/blob/main/examples/infer/transformers/batch_ddp.sh). Note that you cannot set `--stream true` when performing batch inference.
- DDP/device_map Inference: `infer_backend=transformers` supports parallel inference using DDP/device_map technology. For further details, refer to [here](https://github.com/us-inc/usf-bios/blob/main/examples/infer/transformers/mllm_device_map.sh).
- Inference Acceleration: USF BIOS supports using vllm/sglang/lmdeploy for inference acceleration across the inference, deployment, and evaluation modules by simply adding `--infer_backend vllm/sglang/lmdeploy`. You can refer to [here](https://github.com/us-inc/usf-bios/blob/main/examples/infer/vllm/mllm_ddp.sh).
- Multimodal Models: We provide shell scripts for multi-GPU inference for multimodal models using [transformers](https://github.com/us-inc/usf-bios/blob/main/examples/infer/transformers/mllm_device_map.sh), [vllm](https://github.com/us-inc/usf-bios/blob/main/examples/infer/vllm/mllm_tp.sh), and [lmdeploy](https://github.com/us-inc/usf-bios/blob/main/examples/infer/lmdeploy/mllm_tp.sh).
...
- More Model Types: We also provide inference scripts for [bert](https://github.com/us-inc/usf-bios/blob/main/examples/infer/transformers/bert.sh), [reward_model](https://github.com/us-inc/usf-bios/blob/main/examples/infer/transformers/reward_model.sh), and [prm](https://github.com/us-inc/usf-bios/blob/main/examples/infer/transformers/prm.sh).

**Tips:**

- USF BIOS saves inference results, and you can specify the save path using `--result_path`.
- To output log probabilities, simply specify `--logprobs true` during inference. USF BIOS will save these results. Note that setting `--stream true` will prevent storage of results.
- Using `infer_backend=transformers` supports inference for all models supported by USF BIOS, while `infer_backend=vllm/lmdeploy` supports only a subset of models. Please refer to the documentation for [vllm](https://docs.vllm.ai/en/latest/models/supported_models.html), [sglang](https://docs.sglang.ai/supported_models/generative_models.html) and [lmdeploy](https://lmdeploy.readthedocs.io/en/latest/supported_models/supported_models.html).
...
- If you encounter OOM issues while inferring qwen2-vl/qwen2.5-vl, you can address this by setting `MAX_PIXELS`, `VIDEO_MAX_PIXELS`, and `FPS_MAX_FRAMES`. For more information, refer to [here](https://github.com/us-inc/usf-bios/blob/main/examples/app/mllm.sh).
- USF BIOS's built-in dialogue templates align with dialogue templates run using transformers. You can refer to [here](https://github.com/us-inc/usf-bios/blob/main/tests/test_align/test_template/test_vision.py) for testing. If there are any misalignments, please feel free to submit an issue or PR for correction.
...
infer_requests = [
    ...
    InferRequest(messages=[{'role': 'user', 'content': '<image><image> What is the difference between these two images?'}],
                 images=['https://placehold.co/400x300/png?text=cat',
                         'https://placehold.co/400x300/png?text=animal']),
    InferRequest(messages=[{'role': 'user', 'content': '<video> Describe the video'}],
                 videos=['https://placehold.co/400x300/mp4?text=video']),
]
resp_list = engine.infer(infer_requests, request_config)
query0 = infer_requests[0].messages[0]['content']
print(f'response0: {resp_list[0].choices[0].message.content}')
...
- For streaming inference using `VllmEngine`, `SglangEngine` and `LmdeployEngine` for inference acceleration, you can refer to [here](https://github.com/us-inc/usf-bios/blob/main/examples/infer/demo.py).
- Multimodal Inference: In addition to the aforementioned multimodal input formats, USF BIOS is compatible with OpenAI's multimodal input format; refer to [here](https://github.com/us-inc/usf-bios/blob/main/examples/infer/demo_mllm.py).
- Grounding Tasks: For performing grounding tasks with multimodal models, you can refer to [here](https://github.com/us-inc/usf-bios/blob/main/examples/infer/demo_grounding.py).
- Multiple LoRA Inference: Refer to [here](https://github.com/us-inc/usf-bios/blob/main/examples/infer/demo_lora.py).
- Agent Inference: Refer to [here](https://github.com/us-inc/usf-bios/blob/main/examples/infer/demo_agent.py).
- Asynchronous Interface: For Python-based inference using `engine.infer_async`, refer to [here](https://github.com/us-inc/usf-bios/blob/main/examples/infer/demo.py).
...
We introduce three methods for invoking the client: using curl, the OpenAI library, and the USF BIOS client.
...
messages = [{'role': 'user', 'content': [
    {'type': 'video', 'video': 'https://placehold.co/400x300/mp4?text=video'},
...
import base64
import requests
resp = requests.get('https://placehold.co/400x300/mp4?text=video')
...
**Method 3: USF BIOS Client**

```python
from usf_bios import InferRequest, InferClient, RequestConfig, InferStats
...
- Multiple LoRA deployment and invocation: Refer to [this link](https://github.com/us-inc/usf-bios/tree/main/examples/deploy/lora).
- Deployment and invocation of the Base model: Refer to [this link](https://github.com/us-inc/usf-bios/tree/main/examples/deploy/client/llm/base).
- More model types: We provide deployment scripts for [bert](https://github.com/us-inc/usf-bios/tree/main/examples/deploy/bert) and [reward_model](https://github.com/us-inc/usf-bios/tree/main/examples/deploy/reward_model).
