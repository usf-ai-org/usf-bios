...
**Version Requirement**: usf-bios>=3.7
...
```bash
# huggingface
huggingface download --dataset Lixiang/ChenShawn-DeepEyes-Datasets-47k

# huggingface
huggingface-cli download ChenShawn/DeepEyes-Datasets-47k --repo-type=dataset
```

There are three parquet files in the dataset. Register them in the `usf/dataset/data/dataset_info.json` file. For each, rename the `prompt` column to `messages`:
...
For registering the reward function and tool call logic locally as used in the paper, you can refer to the [DeepEyes implementation example](https://github.com/us-inc/usf-bios/tree/main/examples/train/grpo/plugin/deepeyes/deepeyes_plugin.py).
