...
from usf_bios.pipelines import prepare_model_template
from usf_bios.utils import is_debug_mode, get_logger, get_n_params_grads, is_master
...
def convert_hf2mcore(args: ExportArguments) -> None:
    ...
    if is_debug_mode():
        logger.info_debug(f'megatron_config: {kwargs}')
    ...
    if not _test_convert_precision:
        ...
        if is_debug_mode():
            logger.info_debug(f'Successfully saved Megatron model weights in `{args.output_dir}`.')
...
def convert_mcore2hf(args: ExportArguments) -> None:
    from usf_bios.megatron import prepare_mcore_model, adapter_state_dict_context
    ...
    if args.to_hf:
        ...
        if is_debug_mode():
            logger.info_debug(f'Successfully saved HF model weights in `{args.output_dir}`.')
        if args.test_convert_precision:
