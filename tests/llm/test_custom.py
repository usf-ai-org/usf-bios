...
from usf_bios.dataset import DatasetMeta, ResponsePreprocessor, load_dataset, register_dataset
...
class TestCustom(unittest.TestCase):
    ...
    def test_custom_model(self):
        ...
        engine = TransformersEngine('nvidia/Nemotron-Mini-4B-Instruct', torch_dtype=torch.float16)
        response = engine.infer([infer_request], request_config)
        usf_bios_response = response[0].choices[0].message.content

        engine.template.template_backend = 'jinja'
        response = engine.infer([infer_request], request_config)
        jinja_response = response[0].choices[0].message.content
        assert usf_bios_response == jinja_response, (f'usf_bios_response: {usf_bios_response}\njinja_response: {jinja_response}')
        print(f'response: {usf_bios_response}')

    def test_custom_dataset(self):
        dataset = load_dataset(['usf_bios/stsb'])[0]
