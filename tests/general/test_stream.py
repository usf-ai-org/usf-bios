...
def test_local_dataset():
    ...
    from usf_bios.utils import git_clone_github
    model_dir = git_clone_github('https://huggingface.co/datasets/usf-bios/sft-mixture')
    dataset = load_dataset(datasets=[f'{model_dir}:firefly'], streaming=True)[0]
    print(next(iter(dataset)))


def test_hub_dataset():
    local_dataset = 'usf_bios/sft-mixture:firefly'
